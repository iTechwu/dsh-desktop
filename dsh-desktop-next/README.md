...
Use **Settings → Desktop** in the official frontend, the tray’s **Desktop settings…** entry, or `CmdOrCtrl+,`. The tray and independent control window remain available if the Host fails. Profile switches, feature changes, and network changes stop the current Host before starting its replacement, interrupting active tasks.

- **Tray and background operation:** open or restore the main window, reload its interface, select a Profile, open a terminal or browser, and reach diagnostics and recovery. Closing the main window keeps the Host and remote connection running when background operation is enabled and a tray is available. Explicit Quit stops the HTTPS edge and Host. Without a usable tray, closing the main window quits rather than leaving an inaccessible process.
- **Desktop preferences:** background operation, macOS transparency, supported Windows Mica, local/LAN access and ports, log level, and separate notifications for completed/failed user turns and background jobs. Material and notification changes apply live. Acrylic stays disabled, as in the existing Desktop; Mica requires Windows build 22621 or newer. The OS must also allow notifications. Notifications contain generic outcomes and only appear while the main window is unfocused.
- **Profiles:** create, switch, open, or remove an inactive Profile. Removed Profiles move into recovery backups; the active and default Profiles cannot be removed. Profiles have separate plugin dependencies, activation lists, patches, and feature switches. Sessions, settings and credentials follow upstream rules and are shared within one Next home; Profiles do not isolate accounts or data.
...
Browser access is disabled by default. Enabling local access provides an authenticated loopback login link; enabling LAN access adds an HTTPS/WSS edge while the Host stays bound to `127.0.0.1`. Ports default to `0` (automatic). Access/port changes require a Host restart. LAN addresses are sampled at startup; restart after a network change.
...
The independent recovery assistant shows the startup error and recent logs even if no Host is running. It offers retry, Profile switching, diagnostics, safe mode, repair, rollback and Quit.
...
The main interface consumes official frontend artifacts without copying chat, settings, or plugin-manager pages. macOS window material, platform markers, the Windows caption menu, and native theme synchronization follow the official implementation. Next contributes its Desktop section through the official `settings.section` slot. That section and the independent control/recovery window share one implementation, and narrow sender-validated IPC exposes only named native actions. Ordinary browsers receive no native Desktop bridge.
...
This is a runnable development package without signed installers, automatic updates, or Stable/Beta data migration. The official distribution's offline Python/Office runtime and skill payloads are not yet integrated. Our enhanced/extended window modes remain deferred. The update section explicitly reports the absent Next release channel; it never installs a Stable/Beta package. Headless Node/Electron checks do not qualify cross-platform installers or visual behavior.
