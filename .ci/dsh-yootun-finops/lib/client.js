  ...
  factory: (require) => { var module = { exports: {} }; var exports = module.exports;
    ...
    const copy = {
      zh: {
        ...
        noData: '当前范围暂无消费', noBudget: '预算未配置', noAlerts: '当前没有待处理告警', noBaseline: '暂无基线', loading: '正在更新模型数据...', refreshError: '刷新失败，当前仍显示上次数据。', retry: '重新加载',
        confirmedAmount: '已确认金额', partialAttention: '部分用量或费用待补全', partialAttentionHint: '当前只展示 Models 已确认的账本数据',
        modelMix: '模型构成', modelTable: '模型明细表', input: '输入 Token', output: '输出 Token', imageInput: '图像 Token', thinking: '推理 Token', cache: '缓存 Token', amount: '费用', avgCost: '单请求费用', share: '费用占比',
        ...
        attention: '需要关注', updated: '更新于', period: '统计范围', attribution: '归属', attributionMember: '当前成员', attributionTeam: '团队汇总', attributionService: '服务主体', attributionUnknown: '归属未提供', timeZone: '时区',
      ...
      en: {
        ...
        noData: 'No spend in this range', noBudget: 'No budget configured', noAlerts: 'No alerts need attention', noBaseline: 'No baseline', loading: 'Refreshing model data...', refreshError: 'Refresh failed. Showing the previous data.', retry: 'Try again',
        confirmedAmount: 'Confirmed amount', partialAttention: 'Some usage or spend needs reconciliation', partialAttentionHint: 'Only ledger data confirmed by Models is shown',
        modelMix: 'Model mix', modelTable: 'Model detail table', input: 'Input tokens', output: 'Output tokens', imageInput: 'Image tokens', thinking: 'Reasoning tokens', cache: 'Cache tokens', amount: 'Cost', avgCost: 'Cost per request', share: 'Cost share',
        ...
        attention: 'Needs attention', updated: 'Updated', period: 'Range', attribution: 'Attribution', attributionMember: 'Current member', attributionTeam: 'Team aggregate', attributionService: 'Service principal', attributionUnknown: 'Not provided', timeZone: 'Timezone',
    ...
    function costLineSegments(points, x, y) {
      ...
      for (const [index, row] of points.entries()) {
        ...
        current.push(`${x(index)},${y(row.cost)}`)
      }
      if (current.length) segments.push(current)
      return segments
    }

    function TrendChart({ series, currency, source, t }) {
      const points = (series || []).map(row => ({ ...row, cost: finite(row.cost) }))
      const values = points.map(row => row.cost).filter(value => value !== null)
      if (!points.length) return h(EmptyState, { title: source?.sourceCompleteness === 'partial' ? t('unavailable') : t('noData'), source: source || { status: 'empty' }, t })
      const width = 720, height = 220, left = 42, right = 12, top = 18, bottom = 32
      const observedCostMax = values.length ? Math.max(...values) : 0
      const costMax = observedCostMax > 0 ? observedCostMax : 1
      const requestMax = Math.max(...points.map(row => finite(row.requests) || 0), 1)
      const plotBottom = height - bottom
      const x = index => points.length <= 1 ? width / 2 : left + index * ((width - left - right) / (points.length - 1))
      const y = value => top + (plotBottom - top) * (1 - value / costMax)
      const requestY = value => top + (plotBottom - top) * (1 - value / requestMax)
      const barWidth = Math.max(3, Math.min(12, ((width - left - right) / Math.max(points.length, 1)) * .48))
      const labelEvery = Math.max(1, Math.ceil(points.length / 8))
      const lines = costLineSegments(points, x, y)
      return h('div', { className: 'yf-chart-wrap' },
        h('div', { className: 'yf-chart-legend' },
          values.length ? h('span', null, h('i', { className: 'yf-legend-line' }), t('cost')) : null,
          h('span', null, h('i', { className: 'yf-legend-bar' }), t('requestCount'))),
        h('svg', { className: 'yf-chart', viewBox: `0 0 ${width} ${height}`, role: 'img', 'aria-label': t('trendTitle') },
          ...[0, .5, 1].map((ratio, index) => h('line', { key: `grid-${index}`, x1: left, x2: width - right, y1: y(costMax * ratio), y2: y(costMax * ratio), className: 'yf-grid-line' })),
          ...points.map((row, index) => { const requests = finite(row.requests); return requests === null ? null : h('rect', { key: `requests-${row.date}`, x: x(index) - barWidth / 2, y: requestY(requests), width: barWidth, height: Math.max(0, plotBottom - requestY(requests)), className: 'yf-chart-bar' }) }),
          ...lines.map((line, index) => h('polyline', { key: `line-${index}`, points: line.join(' '), className: 'yf-chart-line', fill: 'none' })),
          ...points.map((row, index) => row.cost === null ? null : h('circle', { key: row.date, cx: x(index), cy: y(row.cost), r: 4, className: 'yf-chart-dot' })),
          ...points.map((row, index) => index % labelEvery === 0 || index === points.length - 1 ? h('text', { key: `label-${row.date}`, x: x(index), y: height - 10, className: 'yf-chart-label', textAnchor: 'middle' }, String(row.date || '').slice(5)) : null)),
    ...
    function ModelTable({ models, currency, t }) {
      ...
      const showImageTokens = rows.some(row => finite(row.imageInputTokens) !== null)
      return h('div', { className: 'yf-table-scroll' },
        h('table', { className: 'yf-table' },
          h('thead', null, h('tr', null, h('th', null, t('modelsTab')), h('th', null, t('amount')), h('th', null, t('requestCount')), h('th', null, t('tokens')), h('th', null, t('input')), showImageTokens ? h('th', null, t('imageInput')) : null, h('th', null, t('cache')), h('th', null, t('output')), h('th', null, t('thinking')), h('th', null, t('avgCost')), h('th', null, t('successRate')))),
          h('tbody', null, ...rows.map(row => h('tr', { key: row.id },
            ...
            showImageTokens ? h('td', null, formatNumber(row.imageInputTokens) || '—') : null,
            h('td', null, formatNumber(sumAvailable(row.cacheReadTokens, row.cacheWriteTokens)) || '—'),
            h('td', null, formatNumber(row.outputTokens) || '—'),
            h('td', null, formatNumber(row.thinkingTokens) || '—'),
            h('td', null, formatMoney(row.costPerRequest, currency) || '—'),
            h('td', null, percent(row.successfulRequests, row.requests) || '—'))))))
    }
    ...
    function AlertPanel({ alerts, source, t }) {
      const supplied = Array.isArray(alerts) ? alerts : []
      const rows = supplied.length ? supplied : source?.sourceCompleteness === 'partial' ? [{ id: 'source-partial', title: t('partialAttention'), status: t('partialAttentionHint') }] : []
    ...
    function Overview({ data, t }) {
      ...
      return h('div', { className: 'yf-overview' },
        h('div', { className: 'yf-status-line' }, h(SourceBadge, { source, t }), h('span', null, `${t('updated')} ${shortTime(source.asOf || data?.refreshedAt)}`), h('span', null, `${t('attribution')}: ${attributionLabel(data?.attribution, t)}`)),
        h('div', { className: 'yf-metrics' }, h(Metric, { icon: 'sparkle', label: t('spend'), value: formatMoney(summary.cost, currency) || t('metricUnavailable'), hint: source.sourceCompleteness === 'partial' ? `${t('confirmedAmount')} · ${t('currency')} ${currency}` : `${t('currency')} ${currency}`, muted: source.status === 'error' }), h(Metric, { icon: 'data', label: t('requests'), value: formatNumber(summary.requests) || '—', hint: `${t('successRate')} ${success || t('noBaseline')}` }), h(Metric, { icon: 'layers', label: t('tokens'), value: formatNumber(summary.totalTokens) || '—', hint: tokenBreakdown(summary, t) }), h(Metric, { icon: 'goal', label: t('budgetUsage'), value: budget.value, hint: budget.hint, muted: budget.muted })),
        h('div', { className: 'yf-main-grid' }, h('section', { className: 'yf-panel' }, h('div', { className: 'yf-panel-heading' }, h('div', null, h('h2', null, t('trendTitle')), h('p', null, t('trendHint'))), h(Glyph, { name: 'clock' })), source.status === 'error' || source.status === 'unavailable' ? h(EmptyState, { title: t('unavailable'), source, t }) : h(TrendChart, { series: data?.series, currency, source, t })), h(BudgetPanel, { budget: data?.budget, t })),
    ...
    function attributionLabel(value, t) {
      return ({ member: t('attributionMember'), team_only: t('attributionTeam'), service: t('attributionService') })[value] || t('attributionUnknown')
    }

    function tokenBreakdown(summary, t) {
      const cacheTokens = sumAvailable(summary.cacheReadTokens, summary.cacheWriteTokens)
      const parts = [[t('input'), summary.inputTokens], [t('output'), summary.outputTokens], [t('imageInput'), summary.imageInputTokens], [t('thinking'), summary.thinkingTokens], [t('cache'), cacheTokens]]
      return parts.filter(([, value], index) => index < 2 || finite(value) > 0).map(([label, value]) => `${label} ${formatNumber(value) || '—'}`).join(' · ')
    }
    ...
    const css = `
    .yf-button{display:flex;width:36px;height:36px;align-items:center;justify-content:center;gap:8px;border:0;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer}.yf-button:hover{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary)}.yf-wide{width:100%;height:34px;justify-content:flex-start;padding:0 10px}.yf-wide span{font-size:13px}.yf-overlay{position:fixed;inset:0;z-index:510;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}.yf-shell{display:grid;grid-template-rows:auto auto auto 1fr;width:100%;height:100%;overflow:hidden}.yf-header{display:flex;min-height:72px;align-items:center;justify-content:space-between;gap:18px;padding:16px 24px;border-bottom:1px solid var(--dsw-alias-border-l1)}.yf-title-row{display:flex;align-items:baseline;gap:10px}.yf-header h1{margin:0;font-size: 20px;line-height:1.3}.yf-period-label{padding:2px 7px;border-radius:4px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font-size:12px}.yf-header p{margin:4px 0 0;color:var(--dsw-alias-label-secondary);font-size:13px}.yf-header-buttons{display:flex;gap:6px}.yf-icon-button{display:grid;width:36px;height:36px;place-items:center;border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:inherit;cursor:pointer}.yf-icon-button:hover{background:var(--dsw-alias-bg-layer-2)}.yf-icon-button:disabled{opacity:.45;cursor:default}.yf-toolbar{display:flex;align-items:center;gap:16px;padding:12px 28px;border-bottom:1px solid var(--dsw-alias-border-l1)}.yf-range-control{display:inline-flex;padding:2px;border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-1)}.yf-range-control button{min-width:72px;height:30px;padding:0 12px;border:0;border-radius:4px;background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;font-size:12px;cursor:pointer}.yf-range-control button.is-active{background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary-foreground);font-weight:650}.yf-range-small button{min-width:0;height:26px;padding:0 10px;font-size:11px}.yf-toolbar-meta{color:var(--dsw-alias-label-secondary);font-size:12px}.yf-tabs{display:flex;gap:2px;padding:0 28px;border-bottom:1px solid var(--dsw-alias-border-l1);overflow-x:auto}.yf-tabs button{height:42px;padding:0 14px;border:0;border-bottom:2px solid transparent;background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;font-size:13px;white-space:nowrap;cursor:pointer}.yf-tabs button[aria-current]{border-bottom-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary);font-weight:650}.yf-content{overflow:auto;padding:24px 28px 44px}.yf-content.yf-refreshing{opacity:.58;transition:opacity .2s}.yf-overview,.yf-detail-view{display:grid;gap:22px;max-width:1440px;margin:0 auto}.yf-status-line{display:flex;align-items:center;gap:14px;min-height:30px;color:var(--dsw-alias-label-secondary);font-size:12px}.yf-source-badge{display:inline-flex;align-items:center;gap:6px;color:var(--dsw-alias-label-secondary);font-size:12px}.yf-source-ready svg{color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 50%,var(--dsw-alias-label-primary))}.yf-source-warning svg,.yf-source-empty svg{color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))}.yf-source-error svg{color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary))}.yf-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));border-block:1px solid var(--dsw-alias-border-l1)}.yf-metric{display:grid;min-height:112px;align-content:center;gap:8px;padding:16px 18px;border-right:1px solid var(--dsw-alias-border-l1)}.yf-metric:last-child{border-right:0}.yf-metric-label{display:flex;align-items:center;gap:7px;color:var(--dsw-alias-label-secondary);font-size:12px}.yf-metric-label svg{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary))}.yf-metric strong{font-size:26px;line-height:1.12;font-variant-numeric:tabular-nums;overflow-wrap:anywhere}.yf-metric small{color:var(--dsw-alias-label-secondary);font-size:11px;line-height:1.35}.yf-metric-muted strong{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary));font-size:20px}.yf-main-grid,.yf-secondary-grid{display:grid;grid-template-columns:minmax(0,2fr) minmax(300px,1fr);gap:22px}.yf-secondary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.yf-panel{min-width:0;padding:18px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;background:var(--dsw-alias-bg-layer-1)}.yf-panel-heading{display:flex;align-items:flex-start;justify-content:space-between;gap:14px;margin-bottom:16px}.yf-panel-heading h2,.yf-detail-view>h2{margin:0;font-size:15px;line-height:1.35}.yf-panel-heading p{margin:4px 0 0;color:var(--dsw-alias-label-secondary);font-size:12px}.yf-panel-heading>svg{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary))}.yf-chart-wrap{display:grid;gap:14px;container-type:inline-size}.yf-chart-legend{display:flex;justify-content:flex-end;gap:14px;color:var(--dsw-alias-label-secondary);font-size:11px}.yf-chart-legend span{display:inline-flex;align-items:center;gap:5px}.yf-chart-legend i{display:inline-block;width:14px}.yf-legend-line{height:2px;background:var(--dsw-alias-brand-primary)}.yf-legend-bar{height:8px;background:var(--dsw-alias-brand-primary);opacity:.22}.yf-chart{width:100%;height:auto;min-height:190px;overflow:visible}.yf-grid-line{stroke:var(--dsw-alias-border-l1);stroke-width:1}.yf-chart-bar{fill:var(--dsw-alias-brand-primary);opacity:.18}.yf-chart-line{stroke:var(--dsw-alias-brand-primary);stroke-width:2.5}.yf-chart-dot{fill:var(--dsw-alias-bg-layer-1);stroke:var(--dsw-alias-brand-primary);stroke-width:2}.yf-chart-label{fill:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary));font-size:11px;font-size:calc(11px * 720 / (100cqw / 1px))}.yf-chart-table{width:100%;border-collapse:collapse;font-size:11px;font-variant-numeric:tabular-nums}.yf-chart-table th,.yf-chart-table td{padding:7px 4px;border-top:1px solid var(--dsw-alias-border-l1);text-align:right}.yf-chart-table th:first-child,.yf-chart-table td:first-child{text-align:left}.yf-chart-table th{color:var(--dsw-alias-label-secondary);font-weight:500}.yf-trend-toolbar{display:flex;align-items:center;gap:14px;flex-wrap:wrap}.yf-compare{display:flex;align-items:center;flex-wrap:wrap;gap:7px}.yf-compare-label{color:var(--dsw-alias-label-secondary);font-size:12px}.yf-compare-chip{padding:3px 9px;border:1px solid var(--dsw-alias-border-l1);border-radius:999px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);font-size:11px;font-variant-numeric:tabular-nums}.yf-compare-na{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary));font-size:12px}.yf-status-badge{display:inline-flex;align-items:center;gap:5px;padding:2px 8px;border-radius:999px;background:var(--dsw-alias-bg-layer-2);font-size:11px}.yf-status-healthy{color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 50%,var(--dsw-alias-label-primary))}.yf-status-warning{color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))}.yf-status-critical,.yf-status-exceeded{color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary))}.yf-status-unavailable{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary))}.yf-budget-list{display:grid;gap:16px}.yf-budget-item{display:grid;gap:6px}.yf-budget-warning{background:var(--dsw-alias-state-warn-primary)}.yf-budget-critical,.yf-budget-exceeded{background:var(--dsw-alias-state-error-primary)}.yf-budget-value{display:flex;align-items:baseline;gap:7px;margin:24px 0 14px}.yf-budget-value strong{font-size:25px;font-variant-numeric:tabular-nums}.yf-budget-value span{color:var(--dsw-alias-label-secondary);font-size:12px}.yf-bar-track{height:7px;border-radius:4px;background:var(--dsw-alias-bg-layer-2);overflow:hidden}.yf-bar-fill{height:100%;min-width:0;border-radius:4px;background:var(--dsw-alias-brand-primary);transition:width .25s ease}.yf-model-list{display:grid;gap:13px}.yf-model-row{display:grid;gap:6px}.yf-model-head{display:flex;justify-content:space-between;gap:12px;font-size:12px}.yf-model-head strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.yf-model-head span{color:var(--dsw-alias-label-secondary);font-variant-numeric:tabular-nums}.yf-model-meta,.yf-budget-meta{display:flex;justify-content:space-between;gap:12px;margin-top:9px;color:var(--dsw-alias-label-secondary);font-size:11px;font-variant-numeric:tabular-nums}.yf-table{width:100%;border-collapse:collapse;font-size:12px;font-variant-numeric:tabular-nums}.yf-table th,.yf-table td{padding:9px 8px;border-bottom:1px solid var(--dsw-alias-border-l1);text-align:right;white-space:nowrap}.yf-table th:first-child,.yf-table td:first-child{text-align:left}.yf-table th{color:var(--dsw-alias-label-secondary);font-weight:500;font-size:11px}.yf-panel-note{margin:14px 0 0;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:1.5}.yf-alert-list{border-top:1px solid var(--dsw-alias-border-l1)}.yf-alert-row{display:grid;grid-template-columns:20px minmax(0,1fr) 14px;align-items:center;gap:10px;min-height:48px;border-bottom:1px solid var(--dsw-alias-border-l1)}.yf-alert-row>svg:first-child{color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))}.yf-alert-row>svg:last-child{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary))}.yf-alert-row div{display:grid;gap:3px;min-width:0}.yf-alert-row strong{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px}.yf-alert-row span{color:var(--dsw-alias-label-secondary);font-size:11px}.yf-empty,.yf-loading,.yf-fatal{display:grid;min-height:150px;place-items:center;align-content:center;gap:9px;color:var(--dsw-alias-label-secondary);font-size:13px;text-align:center}.yf-empty strong{font-size:13px}.yf-empty details{font-size:11px}.yf-empty summary{cursor:pointer}.yf-empty code{color:var(--dsw-alias-label-secondary);font-size:11px}.yf-empty-error svg{color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary))}.yf-empty-unavailable svg{color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))}.yf-fatal button{min-height:34px;padding:0 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);cursor:pointer}.yf-stale{padding:8px 28px;background:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 10%,transparent);color:var(--dsw-alias-label-secondary);font-size:12px}.yf-privacy{margin:0;color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary));font-size:11px}@media(max-width:900px){.yf-header,.yf-toolbar,.yf-tabs{padding-left:16px;padding-right:16px}.yf-content{padding:18px}.yf-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.yf-metric:nth-child(2){border-right:0}.yf-metric:nth-child(-n+2){border-bottom:1px solid var(--dsw-alias-border-l1)}.yf-main-grid,.yf-secondary-grid{grid-template-columns:1fr}.yf-secondary-grid{gap:16px}.yf-table{font-size:11px}}@media(max-width:480px){.yf-header{min-height:64px;padding-top:12px;padding-bottom:12px}.yf-header h1{font-size:18px}.yf-header p{display:none}.yf-title-row{display:grid;gap:2px}.yf-toolbar{align-items:flex-start;flex-direction:column;gap:9px}.yf-range-control{width:100%}.yf-range-control button{flex:1;min-width:0}.yf-content{padding:14px}.yf-metric{min-height:92px;padding:12px}.yf-metric strong{font-size:21px}.yf-metric-muted strong{font-size:17px}.yf-status-line{align-items:flex-start;flex-wrap:wrap;gap:8px}.yf-panel{padding:14px}.yf-chart-table{font-size:10px}.yf-table th:nth-child(n+5),.yf-table td:nth-child(n+5){display:none}}
    `
