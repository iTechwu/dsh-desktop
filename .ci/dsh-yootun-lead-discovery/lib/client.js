window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) { ctx.effect(() => ctx.locale.register(NS, copy), 'dofe-yootun-lead-discovery: dictionaries'); ctx.effect(() => { window.addEventListener(OVERLAY_EVENT, closeOtherOverlay); return () => window.removeEventListener(OVERLAY_EVENT, closeOtherOverlay) }, 'dofe-yootun-lead-discovery: exclusive-overlay'); ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = '@dofe/dsh-yootun-lead-discovery'; style.textContent = css + distributionLayoutCss + interactionCss; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-lead-discovery: styles'); const t = ctx.locale.bind(NS); ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({ name: 'sidebar.footer.action', id: 'dofe-yootun-lead-discovery', order: 35, inject: () => ({ t }) }, Button)); ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'dofe-yootun-lead-discovery', order: 35, inject: () => ({ t }) }, Overlay)) }
