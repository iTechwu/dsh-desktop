  ...
  factory: (require) => {
    ...
    const copy = {
      zh: {
        ...
        intent: '意向分', city: '城市', budget: '预算', timing: '购车时机', action: '建议动作', noSummary: '暂无摘要', noAction: '暂无建议动作', unavailable: '线索工具暂不可用', error: '读取失败，请稍后重试', refreshError: '刷新失败，当前仍显示上次数据', storeUnavailable: '候选存储暂不可用，请稍后重试', loadMoreError: '更多线索加载失败，当前结果已保留。', loading: '正在读取…', retry: '重新检索', retryLoadMore: '重试加载',
    ...
    function Overlay({ t }) {
      ...
      const loadCandidates = async (force = false) => { if (searchRef.current || pageRef.current || candidateRef.current) return; setTab('saved'); if (candidates && !force) return; const hasPreviousCandidates = candidates?.status === 'ready'; candidateRef.current = true; setCandidateBusy(true); setCandidateError(false); try { const response = await post({ action: 'candidates' }); if (response?.status && response.status !== 'ready') { if (hasPreviousCandidates) setCandidateError(true); else { setCandidates(response); setCandidateItems([]) }; return }; setCandidates(response); setCandidateItems(Array.isArray(response.items) ? response.items : []); setCandidateError(false) } catch { if (hasPreviousCandidates) setCandidateError(true); else { setCandidates({ status: 'error' }); setCandidateItems([]) } } finally { candidateRef.current = false; setCandidateBusy(false) } }
      ...
      const candidateReason = String(candidates?.reason || '').toUpperCase()
      const candidateRefreshBanner = candidateError && candidates?.status === 'ready' ? h('div', { className: 'yl-inline-error', role: 'alert' }, h(IconWarningOutline16, { size: 15 }), h('span', null, t('refreshError')), h('button', { type: 'button', disabled: interactionBusy, onClick: () => loadCandidates(true) }, h(IconRefreshOutline16, { size: 14 }), t('retry'))) : null
      let body
      if (tab === 'saved' && candidateBusy) body = h(StatusMessage, { kind: 'loading', title: t('loading') })
      else if (tab === 'saved' && state === 'error') body = h(StatusMessage, { kind: 'error', title: candidateReason === 'RESULT_STORE_UNAVAILABLE' || candidateReason === 'INTERNAL_ERROR' ? t('storeUnavailable') : t('error'), action: t('retry'), onAction: () => loadCandidates(true) })
      else if (tab === 'saved' && state === 'unavailable') body = h(StatusMessage, { kind: 'unavailable', title: t('unavailable') })
      else if (tab === 'saved' && !candidateItems.length) body = h(React.Fragment, null, candidateRefreshBanner, h(StatusMessage, { kind: 'empty', title: t('candidatesEmpty'), body: t('candidatesBody') }))
      ...
      else body = h(React.Fragment, null, candidateRefreshBanner, h(StatsGrid, { stats, t }), h(Distribution, { stats, t }), h(LeadList, { items: tab === 'saved' ? candidateItems : items, stats, t }), tab === 'discover' && data?.hasMore ? h('div', { className: 'yl-more' }, loadMoreError ? h('span', { role: 'alert' }, t('loadMoreError')) : null, h('button', { type: 'button', onClick: loadMore, disabled: interactionBusy }, loadingMore ? t('loading') : loadMoreError ? t('retryLoadMore') : t('loadMore'))) : null)
    ...
    const candidateErrorCss = `.yl-inline-error{display:flex;align-items:center;gap:9px;margin:0 auto 14px;padding:9px 11px;border:1px solid var(--dsw-alias-state-error-primary);border-radius:6px;background:color-mix(in srgb,var(--dsw-alias-state-error-primary) 8%,var(--dsw-alias-bg-layer-1));color:var(--dsw-alias-state-error-primary);font-size:12px}.yl-inline-error>span{flex:1;min-width:0}.yl-inline-error button{display:inline-flex;flex:none;align-items:center;gap:5px;min-height:30px;padding:0 9px;border:1px solid currentColor;border-radius:5px;background:transparent;color:inherit;font:inherit;cursor:pointer}.yl-inline-error button:disabled{cursor:default;opacity:.45}@media(max-width:680px){.yl-inline-error{align-items:flex-start}}`
    ...
    function apply(ctx) { ctx.effect(() => ctx.locale.register(NS, copy), 'dofe-yootun-lead-discovery: dictionaries'); ctx.effect(() => { window.addEventListener(OVERLAY_EVENT, closeOtherOverlay); return () => window.removeEventListener(OVERLAY_EVENT, closeOtherOverlay) }, 'dofe-yootun-lead-discovery: exclusive-overlay'); ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = '@dofe/dsh-yootun-lead-discovery'; style.textContent = css + distributionLayoutCss + candidateErrorCss + interactionCss; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-lead-discovery: styles'); const t = ctx.locale.bind(NS); ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({ name: 'sidebar.footer.action', id: 'dofe-yootun-lead-discovery', order: 35, inject: () => ({ t }) }, Button)); ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'dofe-yootun-lead-discovery', order: 35, inject: () => ({ t }) }, Overlay)) }
