...
function formatBudget(min, max) {
  const format = value => value === undefined || value === null || value === '' ? '—' : Number.isFinite(Number(value)) && Number(value) >= 0 ? `¥${Math.round(Number(value) / 10000 * 10) / 10}万` : '—'
...
function SearchPanelRedesigned({ query, setQuery, platform, setPlatform, busy, onRun, t, hasResult }) {
  ...
  return h('section', { className: `yl-search-panel${hasResult ? ' yl-search-compact' : ''}` },
    ...
    h('div', { className: 'yl-search-row' },
      h('div', { className: 'yl-platform-picker' }, h('span', { className: 'yl-field-label' }, t('platform')), h('div', { className: 'yl-platform-options', role: 'group', 'aria-label': t('platform') }, [['xiaohongshu-v2', t('platformXhs')], ['douyin', t('platformDouyin')]].map(([value, label]) => h('button', { type: 'button', key: value, disabled: busy, className: platform === value ? 'is-active' : '', 'aria-pressed': platform === value, onClick: () => setPlatform(value) }, label)))),
      h('div', { className: 'yl-query-wrap' }, h('input', { value: query, maxLength: 500, disabled: busy, placeholder: t('placeholder'), onChange: event => setQuery(event.target.value), onKeyDown: event => { if (event.key === 'Enter') void onRun() }, 'aria-label': t('placeholder') }), h('button', { type: 'button', className: 'yl-primary', onClick: () => void onRun(), disabled: busy || !query.trim() }, h(IconSearchOutline16, { size: 16 }), busy ? t('searching') : t('search')))),
    h('p', { className: 'yl-privacy' }, t('privacy')),
    !query ? h('div', { className: 'yl-examples' }, h('span', null, t('examples')), examples.map(example => h('button', { type: 'button', key: example, disabled: busy, onClick: () => setQuery(example) }, example))) : null)
}

function EmptyStateRedesigned({ t, onExample }) {
  return h('section', { className: 'yl-empty-state', role: 'status' },
...
function Overlay({ t }) {
  ...
  // 打开工作台即读取定时任务落库结果，让用户先看到最新线索；深度检索仍由用户主动触发。
  useEffect(() => { if (visible && !candidates && !candidateRef.current) void loadCandidates(false, false) }, [visible])
  if (!visible) return null
  const run = async () => { if (!query.trim() || searchRef.current || pageRef.current || candidateRef.current) return; searchRef.current = true; setBusy(true); setLoadMoreError(false); setTab('discover'); try { const response = await post({ action: 'discover', keyword: query.trim(), platform }); setData(response); setItems(Array.isArray(response.items) ? response.items : []) } catch { setData({ status: 'error' }); setItems([]) } finally { searchRef.current = false; setBusy(false) } }
  const loadMore = async () => { if (searchRef.current || pageRef.current || candidateRef.current || !data?.resultRef || !data?.hasMore) return; pageRef.current = true; setLoadingMore(true); setLoadMoreError(false); try { const response = await post({ action: 'page', resultRef: data.resultRef, cursor: data.nextCursor }); if (response.status !== 'ready') throw new Error(response.reason || 'lead discovery page unavailable'); setData(prev => ({ ...prev, nextCursor: response.nextCursor, hasMore: response.hasMore, stats: { totalAvailable: prev.totalAvailable ?? prev.stats?.totalAvailable ?? null } })); setItems(prev => prev.concat(response.items || [])) } catch { setLoadMoreError(true) } finally { pageRef.current = false; setLoadingMore(false) } }
  const loadCandidates = async (force = false, selectTab = true) => { if (searchRef.current || pageRef.current || candidateRef.current) return; if (selectTab) setTab('saved'); if (candidates && !force) return; candidateRef.current = true; setCandidateBusy(true); try { const response = await post({ action: 'candidates' }); setCandidates(response); setCandidateItems(Array.isArray(response.items) ? response.items : []); if (!data && !selectTab) setData({ status: 'ready', dataSource: 'database', retrievedAt: new Date().toISOString(), items: response.items || [], stats: response.stats, hasMore: false }) } catch { setCandidates({ status: 'error' }); setCandidateItems([]) } finally { candidateRef.current = false; setCandidateBusy(false) } }
  const refresh = () => tab === 'saved' ? loadCandidates(true) : run()
  const interactionBusy = busy || loadingMore || candidateBusy
  ...
  else body = h(React.Fragment, null, h(StatsGrid, { stats, t }), h(Distribution, { stats, t }), h(LeadList, { items: tab === 'saved' ? candidateItems : items, stats, t }), tab === 'discover' && data?.hasMore ? h('div', { className: 'yl-more' }, loadMoreError ? h('span', { role: 'alert' }, t('loadMoreError')) : null, h('button', { type: 'button', onClick: loadMore, disabled: interactionBusy }, loadingMore ? t('loading') : loadMoreError ? t('retryLoadMore') : t('loadMore'))) : null)
  const refreshDisabled = interactionBusy || (tab === 'discover' && !query.trim())
  return h('div', { className: 'yl-overlay', role: 'dialog', 'aria-modal': true, 'aria-labelledby': 'yl-title' }, h('main', { className: 'yl-shell', ref: shellRef, tabIndex: -1 }, h('header', { className: 'yl-header' }, h('div', null, h('div', { className: 'yl-title-row' }, h('h1', { id: 'yl-title' }, t('title')), state === 'ready' ? h('span', { className: 'yl-ready-dot', 'aria-hidden': true }, '●') : null), h('p', null, t('subtitle'))), h('div', { className: 'yl-header-actions' }, h(Tooltip, { label: t('refresh') }, h('button', { type: 'button', 'aria-label': t('refresh'), onClick: refresh, disabled: refreshDisabled }, h(IconRefreshOutline16, { size: 17 }))), h(Tooltip, { label: t('close') }, h('button', { type: 'button', 'aria-label': t('close'), onClick: closeOverlay }, h(IconCloseOutline16, { size: 17 }))))), h('nav', { className: 'yl-tabs', role: 'tablist', 'aria-label': t('title') }, h('button', { type: 'button', role: 'tab', disabled: interactionBusy, 'aria-selected': tab === 'discover', className: tab === 'discover' ? 'is-active' : '', onClick: () => setTab('discover') }, h(IconSearchOutline16, { size: 15 }), t('discover')), h('button', { type: 'button', role: 'tab', disabled: interactionBusy, 'aria-selected': tab === 'saved', className: tab === 'saved' ? 'is-active' : '', onClick: () => loadCandidates() }, h(IconDataOutline16, { size: 15 }), t('saved'), candidates?.count !== null && candidates?.count !== undefined ? h('span', { className: 'yl-tab-count' }, String(candidates.count)) : null)), h('div', { className: 'yl-content', 'aria-busy': interactionBusy }, tab === 'discover' ? h(SearchPanelRedesigned, { query, setQuery, platform, setPlatform, busy: interactionBusy, onRun: run, t, hasResult: Boolean(data) }) : null, body, tab === 'discover' && data?.retrievedAt ? h('p', { className: 'yl-updated' }, h('span', { className: 'yl-data-source' }, t(data.dataSource === 'database' ? 'databaseSource' : 'liveSource')), `${t('updated')}: ${data.retrievedAt}`) : null)))
}
