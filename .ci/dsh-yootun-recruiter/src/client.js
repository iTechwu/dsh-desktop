...
const copy = {
  zh: {
    ...
    overview: '总览', roles: '岗位需求', candidates: '候选人', actions: '待确认', boss: 'BOSS 连接',
  ...
  en: {
    ...
    overview: 'Overview', roles: 'Role requirements', candidates: 'Candidates', actions: 'Approvals', boss: 'BOSS connection',
...
function Overlay({ t }) {
  ...
  const body = error && !data ? h('div', { className: 'yr-empty', role: 'alert' }, 'Unable to load recruiting workspace', h('button', { type: 'button', onClick: () => setRevision(x => x + 1) }, h(IconRefreshOutline16, { size: 14 }), t('refresh'))) : tab === 'overview' ? h(Overview, { data: tData, t }) : tab === 'roles' ? h(Roles, { data: tData, t }) : tab === 'candidates' ? h(Candidates, { data: tData, t }) : tab === 'actions' ? h(Actions, { data: tData, t, onUpdate: update }) : h(Boss, { data: tData, t })
...
function apply(ctx) {
  ...
  ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = '@dofe/dsh-yootun-recruiter'; style.textContent = css; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-recruiter: styles')
