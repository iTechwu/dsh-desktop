...
function Candidates({ data, t, onNavigate }) {
  ...
  return h('div', { className: 'yr-page' },
    ...
    h('div', { className: 'yr-filter-bar' }, h('input', { className: 'yr-filter', value: query, onChange: event => setQuery(event.target.value), placeholder: t('candidates') }), h('select', { className: 'yr-filter', value: stage, onChange: event => setStage(event.target.value) }, h('option', { value: 'all' }, t('stage')), STAGES.map(item => h('option', { value: item, key: item }, item)))),
