...
const copy = {
  zh: {
    ...
    noData: '还没有招聘数据', addRole: '在对话中创建岗位需求', funnel: '招聘漏斗', roleHealth: '岗位健康度', needsAction: '需要你处理', recent: '最近更新', source: '数据来源', ready: '已启用', partial: '部分可用', unavailable: '待连接', error: '异常', empty: '暂无数据', sample: '样本', updated: '更新时间',
    ...
    loginTitle: '使用官方页面扫码登录', loginBody: '在内置窗口中自行登录并操作官方页面；Yootun-Agent 不读取页面会话、登录凭证和二维码。', openBoss: '打开 BOSS 直聘', openInApp: '在内置浏览器打开', openExternal: '在系统浏览器打开', webUseHint: '登录与操作都在官方页面完成；招聘数据通过 Agent 对话沉淀，不会自动抓取页面。', syncNow: '开始同步', syncHint: '同步岗位、候选人阶段与面试记录；敏感信息不会进入知识库。', lastSync: '最近同步', syncRecords: '同步记录', syncWaiting: '等待用户完成官方登录',
    knowledgeTitle: '企业 HR 知识空间', knowledgeBody: '每个企业统一沉淀岗位画像、面试反馈、招聘复盘和流程规范，不需要单独配置空间。', knowledgeSpaces: '企业 HR 空间', knowledgeDocs: '文档', knowledgeMemories: '经验记忆', knowledgePending: '待确认沉淀', knowledgeAction: '沉淀到 HR 知识空间', knowledgeOpen: '打开企业知识',
    ...
    stage: '阶段', stageSourced: '已入库', stageScreening: '筛选中', stageInterview: '面试中', stageOffer: '待入职', stageHired: '已入职', stageArchived: '已归档', evidence: '岗位相关证据', concerns: '待核实问题', score: '匹配度', days: '天',
    roleDraft: '草稿', roleActive: '招聘中', rolePaused: '已暂停', roleClosed: '已关闭', employmentFullTime: '全职', employmentPartTime: '兼职', employmentContract: '合同制', employmentIntern: '实习', feedbackNone: '暂无反馈', feedbackPending: '待反馈', feedbackCompleted: '已反馈', unknown: '未知',
  ...
  en: {
    ...
    noData: 'No recruiting data yet', addRole: 'Create a role requirement in chat', funnel: 'Hiring funnel', roleHealth: 'Role health', needsAction: 'Needs your attention', recent: 'Recently updated', source: 'Data source', ready: 'Enabled', partial: 'Partially available', unavailable: 'Needs connection', error: 'Error', empty: 'No data', sample: 'Sample', updated: 'Updated',
    ...
    loginTitle: 'Scan on the official login page', loginBody: 'Log in and operate the official pages inside the built-in window; Yootun-Agent never reads the page session, credentials, or QR code.', openBoss: 'Open BOSS Zhipin', openInApp: 'Open in app browser', openExternal: 'Open in system browser', webUseHint: 'Login and actions happen on the official pages; recruiting data is captured through Agent conversations, never by scraping.', syncNow: 'Start sync', syncHint: 'Sync roles, candidate stages, and interview notes. Sensitive data never enters the knowledge base.', lastSync: 'Last sync', syncRecords: 'Sync records', syncWaiting: 'Complete login on the official page',
    knowledgeTitle: 'Enterprise HR knowledge space', knowledgeBody: 'Each enterprise gets one managed space for role profiles, interview feedback, hiring retrospectives, and process playbooks.', knowledgeSpaces: 'Enterprise HR space', knowledgeDocs: 'Documents', knowledgeMemories: 'Memories', knowledgePending: 'Pending curation', knowledgeAction: 'Save to HR space', knowledgeOpen: 'Open enterprise knowledge',
    ...
    stage: 'Stage', stageSourced: 'Sourced', stageScreening: 'Screening', stageInterview: 'Interview', stageOffer: 'Offer', stageHired: 'Hired', stageArchived: 'Archived', evidence: 'Role-related evidence', concerns: 'Open questions', score: 'Match score', days: 'days',
    roleDraft: 'Draft', roleActive: 'Active', rolePaused: 'Paused', roleClosed: 'Closed', employmentFullTime: 'Full-time', employmentPartTime: 'Part-time', employmentContract: 'Contract', employmentIntern: 'Internship', feedbackNone: 'No feedback', feedbackPending: 'Feedback pending', feedbackCompleted: 'Feedback received', unknown: 'Unknown',
...
function roleStatusText(value, t) { return enumText(value, { draft: 'roleDraft', active: 'roleActive', paused: 'rolePaused', closed: 'roleClosed' }, t) }
function employmentText(value, t) { return enumText(value, { full_time: 'employmentFullTime', fulltime: 'employmentFullTime', part_time: 'employmentPartTime', parttime: 'employmentPartTime', contract: 'employmentContract', contractor: 'employmentContract', intern: 'employmentIntern', internship: 'employmentIntern' }, t) }
function feedbackText(value, t) { return enumText(value, { none: 'feedbackNone', pending: 'feedbackPending', awaiting_feedback: 'feedbackPending', completed: 'feedbackCompleted', received: 'feedbackCompleted' }, t) }
function SourceBadge({ label, state, t, onClick, disabled }) {
  const status = sourceState(state)
  const labelText = status === 'ready' ? t('ready') : status === 'empty' ? t('empty') : status === 'partial' ? t('partial') : status === 'error' ? t('error') : t('unavailable')
  ...
  return h('div', { className: 'yr-page' },
    ...
    h('div', { className: 'yr-dashboard-grid' },
      h('section', { className: 'yr-panel' },
        ...
        roles.length ? roles.map(role => { const days = staleDays(role); const stale = days !== null && days >= 14; return h('div', { className: 'yr-health-row', key: role.id }, h('div', null, h('strong', null, role.title), h('span', null, `${role.department} · ${role.location}`)), h('span', { className: stale ? 'yr-health-warn' : 'yr-health-good' }, stale ? `${days} ${t('stale')}` : roleStatusText(role.status, t))) }) : h('div', { className: 'yr-empty yr-empty-compact', role: 'status' }, t('noData')),
      ),
...
function stripBullet(value) { return String(value || '').replace(/^[-*•\d.、)]+\s*/, '').trim() }
function draftFromText(input) {
  const lines = String(input || '').split(/\r?\n/).map(stripBullet).filter(Boolean)
  const title = (lines[0] || '待命名岗位').replace(/^岗位需求[:：]?\s*/, '').slice(0, 80)
  const skillLine = lines.find(line => /技能|要求|任职|能力/.test(line)) || ''
  const requiredSkills = skillLine.split(/[、,，；;]/).map(stripBullet).filter(item => item.length > 1 && item.length < 32).slice(0, 6)
...
function Roles({ data, t, onUpdate, busy }) {
  ...
  const onFile = async event => {
    ...
    if (!file) return
    if (!/\.(txt|md|markdown|csv)$/i.test(file.name) && !String(file.type || '').startsWith('text/')) { setMessage(t('fileReadError')); return }
...
function Candidates({ data, t, onNavigate, busy }) {
  ...
  return h('div', { className: 'yr-page' },
    ...
    h('div', { className: 'yr-pipeline-summary' }, counts.length ? counts.map(item => h('div', { className: 'yr-summary-item', key: item.stage }, h('span', { className: 'yr-summary-label' }, stageText(item.stage, t)), h('strong', { className: 'yr-summary-value' }, String(item.count)))) : h('div', { className: 'yr-summary-item' }, h('span', { className: 'yr-summary-label' }, t('candidates')), h('strong', { className: 'yr-summary-value' }, '0'))),
    h('div', { className: 'yr-filter-bar' }, h('input', { className: 'yr-filter', value: query, disabled: busy, onChange: event => setQuery(event.target.value), placeholder: t('candidates'), 'aria-label': t('candidates') }), h('select', { className: 'yr-filter', value: stage, disabled: busy, onChange: event => setStage(event.target.value), 'aria-label': t('stage') }, h('option', { value: 'all' }, t('stage')), STAGES.map(item => h('option', { value: item, key: item }, stageText(item, t))))),
    filtered.length ? h('div', { className: 'yr-pipeline' }, STAGES.filter(item => filtered.some(candidate => candidate.stage === item)).map(item => h('section', { className: 'yr-stage', key: item }, h('div', { className: 'yr-stage-title' }, h('strong', null, stageText(item, t)), h('span', null, String(filtered.filter(candidate => candidate.stage === item).length))), filtered.filter(candidate => candidate.stage === item).map(candidate => h('article', { className: 'yr-card yr-candidate', key: candidate.id }, h('div', { className: 'yr-card-head' }, h('strong', null, candidate.displayName), candidate.matchScore === undefined ? null : h('span', { className: 'yr-score' }, formatMatchScore(candidate.matchScore))), h('p', null, list(candidate.evidence).join(' · ') || t('empty')), h('p', { className: 'yr-muted' }, `${t('concerns')}: ${list(candidate.concerns).join(' · ') || '—'}`), h('small', { className: 'yr-muted' }, `${t('status')}: ${feedbackText(candidate.feedbackStatus || 'none', t)}`)))))) : h('section', { className: 'yr-empty yr-empty-main', role: 'status' }, h('div', { className: 'yr-empty-icon' }, h(IconUserOutline16, { size: 20 })), h('strong', null, t('noData')), h('span', null, t('syncHint')), h('button', { type: 'button', className: 'yr-primary yr-empty-action', disabled: busy, onClick: () => onNavigate?.('boss') }, h(IconLinkOutline16, { size: 14 }), t('boss'))),
  )
}
...
const css = `.yr-sidebar-button{box-sizing:border-box;display:flex;width:36px;height:36px;align-items:center;justify-content:center;gap:8px;border:0;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer}.yr-sidebar-button:hover{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary)}.yr-sidebar-wide{width:100%;height:34px;justify-content:flex-start;padding:0 10px}.yr-sidebar-wide span{font-size:13px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.yr-overlay{position:fixed;inset:0;z-index:510;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}.yr-shell{display:grid;grid-template-rows:auto auto minmax(0,1fr);width:100%;height:100%;overflow:hidden}.yr-header{display:flex;min-height:72px;min-height:72px;align-items:center;justify-content:space-between;gap:16px;padding:16px 24px;border-bottom:1px solid var(--dsw-alias-border-l1)}.yr-header h1{margin:0;font-size:20px;letter-spacing:0}.yr-header p{margin:4px 0 0;color:var(--dsw-alias-label-secondary);font-size:13px}.yr-header-buttons{display:flex;gap:6px}.yr-icon{display:grid;width:36px;height:36px;place-items:center;border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:inherit;cursor:pointer}.yr-tabs{display:flex;gap:2px;padding:0 24px;border-bottom:1px solid var(--dsw-alias-border-l1);overflow-x:auto}.yr-tabs button{height:42px;padding:0 14px;border:0;border-bottom:2px solid transparent;background:transparent;color:var(--dsw-alias-label-secondary);font:inherit;font-size:13px;white-space:nowrap;cursor:pointer}.yr-tabs button[data-active=true]{border-bottom-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary);font-weight:650}.yr-content{min-height:0;overflow:auto;padding:22px 24px 40px}.yr-page{display:grid;width:100%;max-width:1160px;margin:0 auto;align-content:start;gap:18px}.yr-source-row{display:flex;flex-wrap:wrap;gap:8px}.yr-source{display:inline-flex;align-items:center;gap:6px;padding:6px 9px;border:1px solid var(--dsw-alias-border-l1);border-radius:999px;color:var(--dsw-alias-label-secondary);font-size:11px}.yr-source i,.yr-status i{width:7px;height:7px;border-radius:50%;background:var(--dsw-alias-label-tertiary)}.yr-source-ready{color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 50%,var(--dsw-alias-label-primary))}.yr-source-ready i{background:var(--dsw-alias-state-success-primary)}.yr-source-error{color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary))}.yr-source-error i{background:var(--dsw-alias-state-error-primary)}.yr-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));border-block:1px solid var(--dsw-alias-border-l1)}.yr-metric{display:grid;min-height:82px;align-content:center;gap:5px;padding:12px 16px;border-right:1px solid var(--dsw-alias-border-l1)}.yr-metric:last-child{border-right:0}.yr-metric span,.yr-muted{color:var(--dsw-alias-label-secondary);font-size:12px}.yr-metric strong{font-size:24px;font-variant-numeric:tabular-nums}.yr-dashboard-grid{display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,.85fr);gap:18px}.yr-panel,.yr-hero-panel{display:grid;align-content:start;gap:12px;padding:16px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;background:var(--dsw-alias-bg-layer-1)}.yr-panel-title,.yr-heading,.yr-card-head,.yr-stage-title,.yr-health-row,.yr-recent-row{display:flex;align-items:center;justify-content:space-between;gap:12px}.yr-panel-title h2,.yr-heading h2,.yr-panel h2{margin:0;font-size:14px;letter-spacing:0}.yr-count{font-size:18px}.yr-funnel{display:grid;gap:11px}.yr-funnel-row{display:grid;grid-template-columns:78px minmax(0,1fr) 34px 40px;align-items:center;gap:8px;font-size:12px}.yr-funnel-row>span{text-transform:capitalize;color:var(--dsw-alias-label-secondary)}.yr-funnel-row small{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary));text-align:right}.yr-bar-track{height:7px;overflow:hidden;border-radius:99px;background:var(--dsw-alias-bg-layer-3)}.yr-bar{display:block;height:100%;border-radius:99px;background:var(--dsw-alias-brand-primary)}.yr-list{display:grid;gap:0}.yr-action,.yr-card{display:grid;gap:8px;padding:14px 0;border-top:1px solid var(--dsw-alias-border-l1)}.yr-action:first-child,.yr-card:first-child{border-top:0}.yr-action-main{display:grid;gap:4px;min-width:0}.yr-action-main strong,.yr-action-main span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.yr-action-main span,.yr-card p,.yr-note,.yr-hero-panel p,.yr-knowledge-teaser p{margin:0;color:var(--dsw-alias-label-secondary);font-size:12px;line-height:1.6}.yr-action-buttons,.yr-sync-actions,.yr-knowledge-actions{display:flex;flex-wrap:wrap;gap:7px}.yr-action-buttons button,.yr-empty button,.yr-sync-actions button,.yr-knowledge-actions button,.yr-secondary{display:inline-flex;align-items:center;gap:6px;min-height:32px;padding:0 10px;border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:inherit;font:inherit;text-decoration:none;cursor:pointer}.yr-action-buttons button:first-child,.yr-knowledge-actions button{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-brand-primary)}.yr-action-buttons button:disabled,.yr-knowledge-actions button:disabled,.yr-sync-actions button:disabled{opacity:.45;cursor:default}.yr-status{display:inline-flex;align-items:center;gap:6px;color:var(--dsw-alias-label-secondary);font-size:11px}.yr-status-awaiting_confirmation i{background:var(--dsw-alias-state-warn-primary)}.yr-status-confirmed_pending_adapter i,.yr-status-succeeded i{background:var(--dsw-alias-state-success-primary)}.yr-status-failed i,.yr-status-requires_user_login i{background:var(--dsw-alias-state-error-primary)}.yr-status-succeeded{color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 50%,var(--dsw-alias-label-primary))}.yr-status-failed,.yr-status-requires_user_login{color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary))}.yr-status-dismissed{color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary))}.yr-health-row{padding:10px 0;border-top:1px solid var(--dsw-alias-border-l1)}.yr-health-row:first-of-type{border-top:0}.yr-health-row div{display:grid;gap:3px}.yr-health-row span{color:var(--dsw-alias-label-secondary);font-size:11px}.yr-health-good{color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 50%,var(--dsw-alias-label-primary))!important}.yr-health-warn{color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))!important}.yr-mini-stats{display:flex;flex-wrap:wrap;gap:8px}.yr-mini-stats span{padding:6px 8px;border:1px solid var(--dsw-alias-border-l1);border-radius:5px;color:var(--dsw-alias-label-secondary);font-size:11px}.yr-empty{display:grid;min-height:180px;place-items:center;align-content:center;gap:8px;color:var(--dsw-alias-label-secondary);font-size:13px;text-align:center}.yr-empty-compact{min-height:96px}.yr-empty span,.yr-subheading{margin:0;color:color-mix(in srgb,var(--dsw-alias-label-tertiary) 75%,var(--dsw-alias-label-primary));font-size:12px}.yr-heading{align-items:flex-start}.yr-heading>div{display:grid;gap:4px}.yr-pill{padding:4px 7px;border-radius:4px;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-secondary);font-size:11px}.yr-pill-active{color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 50%,var(--dsw-alias-label-primary))}.yr-role-meta{display:flex;flex-wrap:wrap;gap:8px;color:var(--dsw-alias-label-secondary);font-size:11px}.yr-pipeline{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px}.yr-stage{display:grid;align-content:start;gap:4px;min-width:0;padding:12px;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;background:var(--dsw-alias-bg-layer-1)}.yr-stage-title{padding-bottom:7px;color:var(--dsw-alias-label-secondary);font-size:12px}.yr-candidate{padding:12px 0}.yr-score{color:var(--dsw-alias-brand-primary);font-weight:650}.yr-hero-panel{grid-template-columns:auto 1fr auto;align-items:start}.yr-hero-icon{display:grid;width:40px;height:40px;place-items:center;border-radius:7px;background:var(--dsw-alias-bg-layer-2)}.yr-hero-panel h2{font-size:17px}.yr-boss-panel{align-items:center}.yr-sync-grid{display:grid;grid-template-columns:minmax(0,.7fr) minmax(0,1.3fr);gap:18px}.yr-sync-number{font-size:28px;font-weight:650}.yr-knowledge-actions{justify-content:flex-end}.yr-primary{display:inline-flex;align-items:center;gap:7px;min-height:36px;padding:0 13px;border-radius:6px;background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary-foreground,#fff);font:inherit;text-decoration:none}.yr-icon:disabled{opacity:.45;cursor:default}.yr-loading{grid-auto-flow:column;justify-content:center}.yr-spinner{width:18px;height:18px;border:2px solid var(--dsw-alias-border-l2);border-top-color:var(--dsw-alias-brand-primary);border-radius:50%;animation:yr-spin .8s linear infinite}@keyframes yr-spin{to{transform:rotate(360deg)}}@media(max-width:900px){.yr-dashboard-grid,.yr-sync-grid{grid-template-columns:1fr}.yr-pipeline{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:800px){.yr-header,.yr-tabs{padding-left:16px;padding-right:16px}.yr-content{padding:18px 16px 32px}.yr-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.yr-metric:nth-child(2){border-right:0}.yr-metric:nth-child(-n+2){border-bottom:1px solid var(--dsw-alias-border-l1)}.yr-hero-panel{grid-template-columns:auto 1fr}.yr-hero-panel>.yr-source{grid-column:1/-1}.yr-card-head{align-items:flex-start}.yr-action{align-items:flex-start;flex-direction:column}.yr-action-buttons{width:100%}.yr-action-buttons button{flex:1;justify-content:center}}@media(max-width:560px){.yr-header h1{font-size:17px}.yr-header p{display:none}.yr-metric{min-height:72px;padding:10px}.yr-metric strong{font-size:20px}.yr-pipeline{grid-template-columns:1fr}.yr-funnel-row{grid-template-columns:65px minmax(0,1fr) 28px 36px}.yr-hero-panel{grid-template-columns:1fr}.yr-hero-icon{display:none}}`
const spacingCss = `
.yr-status-adapter_pending i{background:var(--dsw-alias-state-success-primary)}
.yr-overlay{--yr-space-1:4px;--yr-space-2:8px;--yr-space-3:12px;--yr-space-4:16px;--yr-space-5:20px;--yr-space-6:24px}
.yr-header{min-height:72px;padding:var(--yr-space-4) var(--yr-space-6)}
.yr-tabs{gap:var(--yr-space-1);padding:0 var(--yr-space-6)}
.yr-tabs button{height:44px;padding:0 var(--yr-space-4)}
.yr-content{padding:var(--yr-space-5) var(--yr-space-6) 40px}
.yr-page{max-width:1160px;gap:var(--yr-space-5)}
.yr-source-row{gap:var(--yr-space-2)}
.yr-source{min-height:30px;box-sizing:border-box;padding:0 10px}
.yr-source-button{font:inherit;background:transparent;cursor:pointer}
.yr-source-button:hover{border-color:var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary)}
.yr-source-button:focus-visible,.yr-filter:focus-visible,.yr-intake-textarea:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:2px}
.yr-dashboard-grid,.yr-sync-grid{gap:var(--yr-space-5)}
.yr-header-buttons,.yr-action-buttons,.yr-sync-actions,.yr-knowledge-actions{gap:var(--yr-space-2)}
.yr-metric{gap:var(--yr-space-2);padding:var(--yr-space-3) var(--yr-space-4)}
.yr-panel,.yr-hero-panel{gap:var(--yr-space-3);padding:var(--yr-space-5)}
.yr-panel-title,.yr-heading{gap:var(--yr-space-3)}
.yr-funnel,.yr-funnel-row{gap:var(--yr-space-3)}
.yr-action,.yr-card{gap:var(--yr-space-2);padding:var(--yr-space-4) 0}
.yr-role-meta,.yr-mini-stats{gap:var(--yr-space-2)}
.yr-health-row{padding:var(--yr-space-3) 0}
.yr-mini-stats span{padding:var(--yr-space-2) var(--yr-space-3)}
.yr-empty{min-height:140px;gap:var(--yr-space-2)}
.yr-empty-main{min-height:180px;padding:var(--yr-space-6)}
.yr-empty-icon{display:grid;width:40px;height:40px;place-items:center;border:1px solid var(--dsw-alias-border-l1);border-radius:8px;color:var(--dsw-alias-label-secondary)}
.yr-empty-action{margin-top:var(--yr-space-2)}
.yr-workbench-toolbar{align-items:center}
.yr-workbench-toolbar>div{display:grid;gap:var(--yr-space-1)}
.yr-intake-panel{background:var(--dsw-alias-bg-layer-1)}
.yr-intake-grid{display:grid;grid-template-columns:minmax(220px,.65fr) minmax(0,1.35fr);gap:var(--yr-space-4);align-items:stretch}
.yr-dropzone{display:grid;min-height:180px;place-items:center;align-content:center;gap:var(--yr-space-2);padding:var(--yr-space-5);border:1px dashed var(--dsw-alias-border-l2);border-radius:8px;color:var(--dsw-alias-label-secondary);text-align:center;cursor:pointer}
.yr-dropzone:hover{border-color:var(--dsw-alias-brand-primary);color:var(--dsw-alias-label-primary)}
.yr-dropzone strong{color:var(--dsw-alias-label-primary);font-size:13px}
.yr-dropzone span{font-size:12px}
...
.yr-summary-item{display:flex;min-width:92px;align-items:baseline;justify-content:space-between;gap:var(--yr-space-3);padding:var(--yr-space-3) var(--yr-space-4);border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-1)}
.yr-summary-label{color:var(--dsw-alias-label-secondary);font-size:12px;text-transform:capitalize}
.yr-summary-value{font-size:18px;font-variant-numeric:tabular-nums}
.yr-filter-bar{display:flex;gap:var(--yr-space-3)}
...
.yr-inline-error{display:flex;width:100%;max-width:1160px;box-sizing:border-box;align-items:center;justify-content:space-between;gap:var(--yr-space-3);margin:0 auto;padding:10px 12px;border:1px solid var(--dsw-alias-state-error-primary);border-radius:6px;color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary));font-size:13px}
...
`
const statusCss = '.yr-source-partial{color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))}.yr-source-partial i{background:var(--dsw-alias-state-warn-primary)}'
function apply(ctx) { ctx.effect(() => ctx.locale.register(NS, copy), 'dofe-yootun-recruiter: dictionaries'); ctx.effect(() => { window.addEventListener(OVERLAY_EVENT, closeOtherOverlay); return () => window.removeEventListener(OVERLAY_EVENT, closeOtherOverlay) }, 'dofe-yootun-recruiter: exclusive-overlay'); ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = '@dofe/dsh-yootun-recruiter'; style.textContent = css + spacingCss + statusCss; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-recruiter: styles'); const t = ctx.locale.bind(NS); ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({ name: 'sidebar.footer.action', id: 'dofe-yootun-recruiter', order: 20, inject: () => ({ t }) }, SidebarButton)); ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'dofe-yootun-recruiter', order: 20, inject: () => ({ t }) }, Overlay)) }
exports.apply = apply
