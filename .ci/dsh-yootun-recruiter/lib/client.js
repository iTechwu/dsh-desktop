  ...
  factory: (require) => {
    ...
    const copy = {
      zh: {
        ...
        overview: '总览', roles: '岗位需求', candidates: '候选人', actions: '待确认', boss: 'BOSS 连接', loading: '正在读取招聘工作台…', retry: '重新加载', loadError: '招聘工作台加载失败',
      ...
      en: {
        ...
        overview: 'Overview', roles: 'Role requirements', candidates: 'Candidates', actions: 'Approvals', boss: 'BOSS connection', loading: 'Loading recruiting workspace…', retry: 'Try again', loadError: 'Could not load recruiting workspace',
    ...
    function Overlay({ t }) {
      ...
      const [tab, setTab] = useState('overview'); const [data, setData] = useState(null); const [error, setError] = useState(false); const [loading, setLoading] = useState(false); const [revision, setRevision] = useState(0)
      useEffect(() => { if (!visible) return undefined; const controller = new AbortController(); setError(false); setLoading(true); void load(controller.signal).then(value => { setData(value); setError(false) }).catch(e => { if (e?.name !== 'AbortError') setError(true) }).finally(() => { if (!controller.signal.aborted) setLoading(false) }); return () => controller.abort() }, [visible, revision])
