window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function MemoryRow({ item, t, onGraph, onConfirm, onForget, mutation }) {
      const status = String(item.status || "CANDIDATE").toUpperCase();
      const activeMutation = mutation?.id === item.id;
      const title =
      ...
      return h(
        ...
        h(
          ...
          status === "CANDIDATE" && onConfirm
            ? h(
                "button",
                { type: "button", disabled: Boolean(mutation), onClick: () => onConfirm(item) },
                h(IconCheckOutline16, { size: 14 }),
                activeMutation && mutation.action === "confirm" ? t("processing") : t("confirm"),
              )
            : null,
          ["CANDIDATE", "CONFIRMED"].includes(status) && onForget
            ? h(
                ...
                activeMutation && mutation.action === "forget" ? t("processing") : t("forget"),
              )
            : null,
        ),
      );
    }
    function Overview({ data, t, onGraph, onTemplate, onConfirm, onForget, mutation }) {
    ...
    function Overlay({ t }) {
      ...
      const confirmMemory = async (item) => {
        ...
        setMutation({ id: item.id, action: "confirm" });
        try {
          await mutate({
      ...
      const forgetMemory = async (item) => {
        ...
        mutationBusyRef.current = true;
        setMutation({ id: item.id, action: "forget" });
        try {
          await mutate({
            action: "forget",
    ...
    const mutationCss = `.yk-row-actions button:disabled{opacity:.45;cursor:default}`;
    function apply(ctx) {
      ctx.effect(
      ...
      ctx.effect(() => {
        ...
        style.textContent = css + stateCss + mutationCss;
