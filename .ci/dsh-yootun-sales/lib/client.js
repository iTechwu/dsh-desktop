window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const copy = {
      zh: { open: '销售协同', title: '销售协同', subtitle: '公开意向发现、线索跟进与确认动作', close: '关闭销售协同', refresh: '刷新', leads: '线索', qualified: '合格线索', dueToday: '今日待跟进', pending: '待确认', empty: '还没有销售线索', actions: '待确认跟进', approve: '确认', dismiss: '撤销', adapter: '已确认，等待适配器', source: '来源', intent: '意向发现', intentPlaceholder: '一句话描述要找的公开意向，例如：长沙新能源汽车改装讨论', intentSearch: '开始检索', intentEmpty: '输入需求后查找公开讨论', intentUnavailable: 'Tools 意向检索暂不可用', intentError: '意向检索失败', confidence: '置信度' },
      en: { open: 'Sales workspace', title: 'Sales workspace', subtitle: 'Public intent discovery, follow-ups, and approvals', close: 'Close sales workspace', refresh: 'Refresh', leads: 'Leads', qualified: 'Qualified', dueToday: 'Due today', pending: 'Awaiting approval', empty: 'No sales leads yet', actions: 'Follow-ups awaiting approval', approve: 'Approve', dismiss: 'Dismiss', adapter: 'Approved, adapter pending', source: 'Source', intent: 'Intent discovery', intentPlaceholder: 'Describe the public intent to find in one sentence', intentSearch: 'Search', intentEmpty: 'Enter a requirement to find public discussions', intentUnavailable: 'Tools intent discovery is unavailable', intentError: 'Intent search failed', confidence: 'Confidence' },
    }
    let opened = false
    ...
    function Action({ item, t, update }) { return h('article', { className: `ys-action ys-action-${item.status}`, 'data-status': item.status }, h('div', { className: 'ys-action-main' }, h('strong', null, item.summary), h('span', null, item.channel), h('small', null, item.status === 'awaiting_confirmation' ? t('pending') : t('adapter'))), item.status === 'awaiting_confirmation' ? h('div', { className: 'ys-action-buttons' }, h('button', { type: 'button', onClick: () => update({ action: 'confirm_action', id: item.id }) }, h(IconCheckOutline16, { size: 14 }), t('approve')), h('button', { type: 'button', onClick: () => update({ action: 'dismiss_action', id: item.id }) }, h(IconCloseOutline16, { size: 14 }), t('dismiss'))) : null) }
    function Overlay({ t }) {
      ...
      useEffect(() => {
        ...
        setError(false)
        void load(controller.signal).then(setData).catch(cause => {
      ...
      const actions = h('section', { className: 'ys-section' },
        h('h2', null, t('actions')),
        current.actions.filter(item => item.status === 'awaiting_confirmation').map(item => h(Action, { key: item.id, item, t, update })),
      )
      ...
      const content = error && !data
        ? h('div', { role: 'alert', className: 'ys-empty' }, 'Unable to load sales workspace')
        ...
        h('main', { className: 'ys-shell', 'aria-labelledby': 'ys-title' },
          h('header', { className: 'ys-header' },
            ...
            h('div', { className: 'ys-header-buttons' },
              h(Tooltip, { label: t('refresh') }, h('button', { type: 'button', 'aria-label': t('refresh'), onClick: () => setRevision(value => value + 1) }, h(IconRefreshOutline16, { size: 16 }))),
