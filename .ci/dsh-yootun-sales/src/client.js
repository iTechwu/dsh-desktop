...
function IntentSearch({ t, current, update, busy, active }) {
  ...
  const run = () => { if (!query.trim() || busy) return; void update({ action: 'intent_search', query: query.trim() }) }
  const intent = current.intent
  return h('section', { className: 'ys-intent', 'aria-busy': active }, h('div', { className: 'ys-section-heading' }, h('h2', null, t('intent'))), h('div', { className: 'ys-intent-form' }, h('input', { value: query, disabled: busy, maxLength: 500, placeholder: t('intentPlaceholder'), 'aria-label': t('intentPlaceholder'), onChange: event => setQuery(event.target.value), onKeyDown: event => { if (event.key === 'Enter') run() } }), h('button', { type: 'button', disabled: busy || !query.trim(), onClick: run }, h(IconDataOutline16, { size: 14 }), active ? '…' : t('intentSearch'))), !intent ? h('div', { className: 'ys-intent-empty' }, t('intentEmpty')) : intent.status === 'unavailable' ? h('div', { className: 'ys-intent-empty' }, t('intentUnavailable')) : intent.status === 'error' ? h('div', { className: 'ys-intent-empty', role: 'alert' }, t('intentError')) : intent.items?.length ? intent.items.slice(0, 10).map((item, index) => h('article', { className: 'ys-intent-row', key: `${item.sourceUrl || item.topic || 'result'}-${index}` }, h('div', null, h('strong', null, item.topic || item.platform || t('source')), h('span', null, item.sourceUrl || ''), item.intentSignals?.length ? h('small', null, item.intentSignals.join(' · ')) : null), h('b', null, item.confidence === undefined ? '' : `${t('confidence')} ${Math.round(Number(item.confidence) * 100)}%`))) : h('div', { className: 'ys-intent-empty' }, t('intentEmpty')))
}
...
function Action({ item, t, update, busy, active }) { return h('article', { className: `ys-action ys-action-${item.status}`, 'data-status': item.status, 'aria-busy': active }, h('div', { className: 'ys-action-main' }, h('strong', null, item.summary), h('span', null, item.channel), h('small', { role: active ? 'status' : undefined }, active ? t('processing') : statusText(item.status, t))), item.status === 'awaiting_confirmation' ? h('div', { className: 'ys-action-buttons' }, h('button', { type: 'button', disabled: busy, onClick: () => update({ action: 'confirm_action', id: item.id }) }, h(IconCheckOutline16, { size: 14 }), t('approve')), h('button', { type: 'button', disabled: busy, onClick: () => update({ action: 'dismiss_action', id: item.id }) }, h(IconCloseOutline16, { size: 14 }), t('dismiss'))) : null) }
function Overlay({ t }) {
  ...
  useEffect(() => {
    ...
    setError('')
    setLoading(true)
    void load(controller.signal).then(value => { setData(value); setError('') }).catch(cause => {
      if (cause?.name !== 'AbortError') setError('load')
  ...
  const update = async body => {
    ...
    setPendingActionId(body.id || body.action)
    try { setData(await mutate(body)); setError('') } catch { setError('action') } finally { actionBusyRef.current = false; setPendingActionId('') }
  }
  const busy = Boolean(pendingActionId)
  const intent = h(IntentSearch, { t, current, update, busy, active: pendingActionId === 'intent_search' })
  ...
  const actions = h('section', { className: 'ys-section' },
    h('h2', null, t('actions')),
    current.actions.map(item => h(Action, { key: item.id, item, t, update, busy, active: pendingActionId === item.id })),
  )
...
const stateCss = `.ys-shell{outline:0}.ys-action-buttons button:disabled{opacity:.45;cursor:default}.ys-error-banner{display:flex;max-width:1120px;box-sizing:border-box;align-items:center;justify-content:space-between;gap:12px;margin:0 auto 16px;padding:10px 12px;border:1px solid var(--dsw-alias-state-error-primary);color:var(--dsw-alias-state-error-primary);font-size:13px}.ys-error-banner button{flex:none;min-height:28px;border:1px solid currentColor;border-radius:5px;background:transparent;color:inherit;font:inherit;cursor:pointer}`
