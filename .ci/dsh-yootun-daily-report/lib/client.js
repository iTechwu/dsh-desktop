window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const copy = { zh: { open: '昨日工作', title: '昨日工作日报', subtitle: '汇总本机 Agent 会话与工具调用', close: '关闭昨日工作日报', refresh: '刷新', loading: '正在读取昨日工作…', refreshError: '刷新失败，当前保留上次结果。', retry: '重新加载', sessions: '会话', turns: '轮次', completed: '完成', failed: '异常', tools: '工具调用', empty: '昨日没有可汇总的工作记录', unavailable: '本机事件暂不可用', privacy: '仅展示标题、工作区与聚合计数，不展示会话正文。', sources: '数据源状态', local: '本机事件', toolsSource: '工具目录', salesIntent: '销售意向', retrofit: '改装检索', knowledge: '企业知识', sourcePartial: '部分可用', partial: '日报不完整，以下仅统计已成功读取的会话。', coverage: '已读取会话', partialReason: '部分会话未能读取', limitedReason: '达到扫描上限，尚有会话未纳入统计', sourceReady: '已就绪', sourceEmpty: '无数据', sourceUnavailable: '不可用', sourceError: '异常', reason: '原因' }, en: { open: 'Yesterday', title: 'Yesterday at a glance', subtitle: 'Local Agent sessions and tool calls', close: 'Close yesterday report', refresh: 'Refresh', loading: 'Loading yesterday’s work…', refreshError: 'Refresh failed. Keeping the previous report.', retry: 'Try again', sessions: 'Sessions', turns: 'Turns', completed: 'Completed', failed: 'Failed', tools: 'Tool calls', empty: 'No local work recorded yesterday', unavailable: 'Local events unavailable', privacy: 'Titles, workspaces, and aggregates only; conversation text is excluded.', sources: 'Source status', local: 'Local events', toolsSource: 'Tool catalog', salesIntent: 'Sales intent', retrofit: 'Retrofit', knowledge: 'Knowledge', sourcePartial: 'Partially available', partial: 'This report is incomplete. Counts include successfully read sessions only.', coverage: 'Sessions read', partialReason: 'Some sessions could not be read', limitedReason: 'Scan limit reached; some sessions are not included', sourceReady: 'Ready', sourceEmpty: 'No data', sourceUnavailable: 'Unavailable', sourceError: 'Error', reason: 'Reason' } }
    let opened = false; let lastTrigger = null; const listeners = new Set(); const emit = () => listeners.forEach(listener => listener()); const setOpened = value => { opened = value; emit() }; const subscribe = listener => { listeners.add(listener); return () => listeners.delete(listener) }; const snapshot = () => opened
    ...
    function statusLabel(status, t) { return status === 'ready' ? t('sourceReady') : status === 'partial' ? t('sourcePartial') : status === 'empty' ? t('sourceEmpty') : status === 'error' ? t('sourceError') : t('sourceUnavailable') }
    ...
    function Overlay({ t }) {
      ...
      useEffect(() => {
        ...
        void fetch(PATH, {
          ...
          .catch(cause => {
            ...
            setError(true)
            setData(current => current || { activity: { status: 'unavailable', reason: 'request_failed' } })
          })
      ...
      const content = loading && !data
        ...
        ? h('div', { className: 'ydr-empty ydr-error', role: 'alert' }, h('strong', null, t('unavailable')), h('button', { type: 'button', disabled: loading, onClick: refresh }, t('retry')))
      ...
      return h(
        ...
        h(
          ...
          h(
            ...
            h(
              ...
              h(
                ...
                h(
                  'button',
                  { type: 'button', 'aria-label': t('close'), onClick: closeOverlay },
          ...
          h(
            ...
            loading && data ? h('div', { className: 'ydr-inline-status', role: 'status' }, h('span', { className: 'ydr-spinner', 'aria-hidden': true }), t('loading')) : null,
            error && data && activity?.status !== 'unavailable' && activity?.status !== 'error' ? h('div', { className: 'ydr-inline-status ydr-inline-error', role: 'alert' }, h('span', null, t('refreshError')), h('button', { type: 'button', disabled: loading, onClick: refresh }, t('retry'))) : null,
            activity?.status === 'partial' ? h('div', { className: 'ydr-inline-status ydr-inline-warning', role: 'status' },
              h('span', null, t('partial'), activity.coverage ? ` ${t('coverage')}: ${activity.coverage.loaded} / ${activity.coverage.total}` : ''),
              activity.coverage?.failed > 0 ? h('button', { type: 'button', disabled: loading, onClick: refresh }, t('retry')) : null) : null,
            h(
              'div',
            ...
            h(
              ...
              ...sourceRows.map(([label, source]) => h(
                ...
                source?.reason ? h('span', { className: 'ydr-source-reason' }, reasonLabel(source.reason, t)) : null,
              )),
            ),
            content,
          ),
    ...
    const css = `.ydr-button{box-sizing:border-box;display:flex;width:36px;height:36px;align-items:center;justify-content:center;border:0;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary);cursor:pointer}.ydr-button:hover{background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary)}.ydr-overlay{position:fixed;inset:0;z-index:510;background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary)}.ydr-shell{display:grid;grid-template-rows:auto minmax(0,1fr);width:100%;height:100%;overflow:hidden}.ydr-header{display:flex;min-height:72px;align-items:center;justify-content:space-between;gap:16px;padding:16px 24px;border-bottom:1px solid var(--dsw-alias-border-l1)}.ydr-header h1{margin:0;font-size:20px;letter-spacing:0}.ydr-header p{margin:4px 0 0;color:var(--dsw-alias-label-secondary);font-size:13px}.ydr-actions{display:flex;gap:6px}.ydr-actions button{display:grid;width:36px;height:36px;place-items:center;border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:inherit;cursor:pointer}.ydr-content{min-height:0;overflow:auto;padding:22px 24px 40px}.ydr-metrics{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));max-width:1120px;margin:0 auto;border-block:1px solid var(--dsw-alias-border-l1)}.ydr-metric{display:grid;min-height:82px;align-content:center;gap:6px;padding:12px 16px;border-right:1px solid var(--dsw-alias-border-l1)}.ydr-metric:last-child{border-right:0}.ydr-metric span{color:var(--dsw-alias-label-secondary);font-size:12px}.ydr-metric strong{font-size:22px;font-variant-numeric:tabular-nums}.ydr-privacy{max-width:1120px;margin:12px auto 24px;color:var(--dsw-alias-label-secondary);font-size:11px;text-align:right}.ydr-sources,.ydr-section{display:grid;gap:0;max-width:1120px;margin:0 auto 24px}.ydr-sources h2{margin:0 0 10px;font-size:14px;letter-spacing:0}.ydr-source-row{display:flex;min-height:42px;align-items:center;gap:10px;flex-wrap:wrap;padding:8px 0;border-top:1px solid var(--dsw-alias-border-l1);font-size:12px}.ydr-source-row>span{min-width:120px}.ydr-source-row b{font-size:11px}.ydr-source-row code{max-width:100%;overflow-wrap:anywhere;color:var(--dsw-alias-label-secondary);font-size:11px}.ydr-source-ready{color:color-mix(in srgb,var(--dsw-alias-state-success-primary) 50%,var(--dsw-alias-label-primary))}.ydr-source-error{color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary))}.ydr-source-unavailable,.ydr-source-empty{color:var(--dsw-alias-label-secondary)}.ydr-row{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:14px 16px;border-top:1px solid var(--dsw-alias-border-l1)}.ydr-row strong{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.ydr-row span{flex:none;color:var(--dsw-alias-label-secondary);font-size:12px}.ydr-empty{display:grid;min-height:180px;place-items:center;max-width:1120px;margin:0 auto;color:var(--dsw-alias-label-secondary);font-size:13px;text-align:center}@media(max-width:800px){.ydr-header,.ydr-content{padding-left:16px;padding-right:16px}.ydr-metrics{grid-template-columns:repeat(2,minmax(0,1fr))}.ydr-metric{border-bottom:1px solid var(--dsw-alias-border-l1)}.ydr-metric:nth-child(2n){border-right:0}.ydr-metric:last-child{border-bottom:0}.ydr-privacy{text-align:left}.ydr-row{align-items:flex-start;flex-direction:column}.ydr-row span{flex:auto}}@media(max-width:480px){.ydr-header h1{font-size:17px}.ydr-header p{display:none}.ydr-metric{min-height:72px;padding:10px}.ydr-metric strong{font-size:18px}}`
    const stateCss = '.ydr-inline-status.ydr-inline-warning{flex-wrap:wrap;border-color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 45%,var(--dsw-alias-border-l1));color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))}.ydr-source-partial{color:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 50%,var(--dsw-alias-label-primary))}.ydr-source-reason{color:var(--dsw-alias-label-secondary);overflow-wrap:anywhere}.ydr-empty{align-content:center;gap:10px}.ydr-empty button,.ydr-inline-status button{min-height:34px;padding:0 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:inherit;font:inherit;cursor:pointer}.ydr-overlay button:disabled{cursor:not-allowed;opacity:.45}.ydr-inline-status{display:flex;max-width:1120px;min-height:38px;align-items:center;gap:9px;margin:0 auto 16px;padding:7px 11px;border:1px solid var(--dsw-alias-border-l1);border-radius:6px;background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-secondary);font-size:12px}.ydr-inline-status>span:first-child{flex:1}.ydr-inline-status>.ydr-spinner{flex:0 0 auto}.ydr-inline-error{border-color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 45%,var(--dsw-alias-border-l1));color:color-mix(in srgb,var(--dsw-alias-state-error-primary) 50%,var(--dsw-alias-label-primary))}.ydr-spinner{width:18px;height:18px;border:2px solid var(--dsw-alias-border-l2);border-top-color:var(--dsw-alias-brand-primary);border-radius:50%;animation:ydr-spin .8s linear infinite}@keyframes ydr-spin{to{transform:rotate(360deg)}}'
    // Session and workspace names remain readable when rows stack in narrow windows.
    const contentLayoutCss = '.ydr-section,.ydr-row{min-width:0}.ydr-row strong,.ydr-row span{max-width:100%;white-space:normal;overflow-wrap:anywhere}'
    function apply(ctx) { ctx.effect(() => ctx.locale.register(NS, copy), 'dofe-yootun-daily-report: dictionaries'); ctx.effect(() => { window.addEventListener(OVERLAY_EVENT, closeOtherOverlay); window.addEventListener('keydown', closeOnEscape); return () => { window.removeEventListener(OVERLAY_EVENT, closeOtherOverlay); window.removeEventListener('keydown', closeOnEscape) } }, 'dofe-yootun-daily-report: overlay-events'); ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = '@dofe/dsh-yootun-daily-report'; style.textContent = css + stateCss + contentLayoutCss; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-daily-report: styles'); const t = ctx.locale.bind(NS); ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({ name: 'sidebar.footer.action', id: 'dofe-yootun-daily-report', order: 50, inject: () => ({ t }) }, Button)); ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'dofe-yootun-daily-report', order: 50, inject: () => ({ t }) }, Overlay)) }
