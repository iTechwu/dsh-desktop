  ...
  factory: (require) => {
    ...
    function Overlay({ t }) {
      ...
      const metricValue = value => activity?.status === 'unavailable' || activity?.status === 'error'
        ? t('sourceUnavailable')
        : String(value ?? 0)
      const refresh = () => { if (loadingRef.current) return; loadingRef.current = true; setLoading(true); setRevision(value => value + 1) }
      const metrics = [
