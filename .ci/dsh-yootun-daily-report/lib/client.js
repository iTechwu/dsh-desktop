window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) { ctx.effect(() => ctx.locale.register(NS, copy), 'dofe-yootun-daily-report: dictionaries'); ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = '@dofe/dsh-yootun-daily-report'; style.textContent = css + stateCss; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-daily-report: styles'); const t = ctx.locale.bind(NS); ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register({ name: 'sidebar.footer.action', id: 'dofe-yootun-daily-report', order: 50, inject: () => ({ t }) }, Button)); ctx.slots.inject('shell.overlay', () => ctx.slots.register({ name: 'shell.overlay', id: 'dofe-yootun-daily-report', order: 50, inject: () => ({ t }) }, Overlay)) }
