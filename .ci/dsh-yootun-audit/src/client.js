...
const ACTION_LABELS = Object.freeze({
  ...
  'xhs.rewrite.completed': '完成小红书仿写', 'xhs.rewrite.cancelled': '取消小红书仿写', 'media.upload.completed': '完成媒体上传',
})
...
function Filters({ state, dispatch, t }) {
  ...
  return h('div', { className: 'ya-filters', 'aria-label': 'audit filters' },
    ...
    h('select', { value: state.filters.outcome, onChange: set('outcome'), 'aria-label': t('allResults') }, h('option', { value: '' }, t('allResults')), h('option', { value: 'succeeded' }, t('succeeded')), h('option', { value: 'partial' }, t('partial')), h('option', { value: 'failed' }, t('failed')), h('option', { value: 'accepted' }, t('accepted')), h('option', { value: 'cancelled' }, t('cancelled'))),
...
const focusCss = `.ya-button:focus-visible,.ya-icon-button:focus-visible,.ya-segments button:focus-visible,.ya-team input:focus-visible,.ya-team select:focus-visible,.ya-filters input:focus-visible,.ya-filters select:focus-visible,.ya-row:focus-visible,.ya-status button:focus-visible,.ya-empty button:focus-visible,.ya-more:focus-visible,.ya-detail-close:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:2px}`
const layoutCss = `.ya-shell{display:flex;flex-direction:column;grid-template-rows:none}.ya-summary{grid-template-columns:repeat(4,minmax(0,1fr))}.ya-workspace,.ya-empty,.ya-skeletons{flex:1;min-height:0}.ya-workspace:not(.has-detail) .ya-detail{display:none}.ya-row>span:last-child{display:flex;align-items:center;gap:5px}.ya-pending{color:var(--dsw-alias-state-warn-primary);font-size:11px;white-space:nowrap}@media(max-width:767px){.ya-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.ya-metric:nth-child(2){border-right:0}.ya-metric:nth-child(-n+2){border-bottom:1px solid var(--dsw-alias-border-l1)}}`
function apply(ctx) {
  ...
  ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = OVERLAY_ID; style.textContent = `${css}${focusCss}${layoutCss}`; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-audit: styles')
