...
const EMPTY_WORKSPACE = Object.freeze({
  status: 'ready', summary: { today: 0, succeeded: 0, abnormal: 0, pendingSync: 0 }, events: [], page: { nextCursor: null },
...
const copy = {
  zh: { open: '操作审计', title: '操作审计', subtitle: '追溯成员与插件实际执行的业务操作', close: '关闭操作审计', refresh: '刷新', self: '本人', team: '团队', today: '今日操作', abnormal: '异常结果', failed: '失败', requiresLogin: '需要用户登录', pendingSync: '待同步', pendingItem: '待同步', search: '搜索操作、对象或成员', allPlugins: '全部插件', allActions: '全部动作', allResults: '全部结果', allTime: '全部时间', time24h: '最近 24 小时', time7d: '最近 7 天', time30d: '最近 30 天', succeeded: '成功', partial: '部分完成', accepted: '已受理', loading: '正在加载审计记录', empty: '尚无可审计操作', filteredEmpty: '没有符合筛选条件的记录', clear: '清除筛选', retry: '立即重试', cached: '当前显示本机缓存', offline: '远端暂不可用，已保留最近记录', auth: '需要重新连接 Models 账户', localError: '本机审计存储暂不可用', time: '发生时间', actor: '操作者', action: '操作', target: '对象', result: '结果', detail: '记录详情', source: '来源', surface: '入口', occurredAt: '发生时间', receivedAt: '接收时间', syncState: '同步状态', changes: '字段变化', effects: '外部结果', errorCode: '错误码', traceId: '追踪 ID', noChanges: '无结构化字段变化', loadMore: '加载更多', chooseTeam: '选择团队后查看记录', searchTeam: '搜索团队', stale: '数据更新于', syncIssue: '条记录尚未同步', quarantine: '条记录需检查', closeDetail: '关闭详情' },
  en: { open: 'Operation audit', title: 'Operation audit', subtitle: 'Trace business operations performed by members and plugins', close: 'Close operation audit', refresh: 'Refresh', self: 'Me', team: 'Team', today: 'Today', abnormal: 'Abnormal', failed: 'Failed', requiresLogin: 'Login required', pendingSync: 'Pending sync', pendingItem: 'Pending', search: 'Search operation, target, or member', allPlugins: 'All plugins', allActions: 'All actions', allResults: 'All results', allTime: 'All time', time24h: 'Last 24 hours', time7d: 'Last 7 days', time30d: 'Last 30 days', succeeded: 'Succeeded', partial: 'Partial', accepted: 'Accepted', loading: 'Loading audit records', empty: 'No auditable operations yet', filteredEmpty: 'No records match these filters', clear: 'Clear filters', retry: 'Retry now', cached: 'Showing local cache', offline: 'Remote unavailable; recent records retained', auth: 'Reconnect your Models account', localError: 'Local audit storage unavailable', time: 'Time', actor: 'Actor', action: 'Operation', target: 'Target', result: 'Result', detail: 'Record details', source: 'Source', surface: 'Surface', occurredAt: 'Occurred', receivedAt: 'Received', syncState: 'Sync status', changes: 'Changes', effects: 'External effects', errorCode: 'Error code', traceId: 'Trace ID', noChanges: 'No structured changes', loadMore: 'Load more', chooseTeam: 'Choose a team to view records', searchTeam: 'Search teams', stale: 'Updated', syncIssue: 'records pending sync', quarantine: 'records need review', closeDetail: 'Close details' },
}
...
function normalizeWorkspace(value) {
  ...
  return {
    ...
    summary: { today: number(summary.today), succeeded: number(summary.succeeded), abnormal: number(summary.abnormal), pendingSync: number(summary.pendingSync) },
...
function initialState() { return { visible: false, loading: false, appending: false, error: '', workspace: null, selectedId: null, scope: 'self', teamId: '', teamQuery: '', teams: [], filters: { ...EMPTY_FILTERS } } }
function reducer(state, action) {
  if (action.type === 'open') return { ...state, visible: true }
  if (action.type === 'close') return { ...state, visible: false, selectedId: null }
  if (action.type === 'loading') return { ...state, loading: action.append !== true, appending: action.append === true, error: '' }
...
async function requestJson(query, signal) { const response = await fetch(`${PATH}?${query}`, { credentials: 'same-origin', redirect: 'error', signal: signal ? AbortSignal.any([signal, AbortSignal.timeout(REQUEST_TIMEOUT_MS)]) : AbortSignal.timeout(REQUEST_TIMEOUT_MS), headers: { Accept: 'application/json' } }); if (!response.ok) throw new Error(`audit_${response.status}`); return response.json() }
async function retrySync() { const response = await fetch(PATH, { method: 'POST', credentials: 'same-origin', redirect: 'error', signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS), headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify({ action: 'retry_sync' }) }); if (!response.ok) throw new Error(`audit_${response.status}`) }
...
function Filters({ state, dispatch, t, busy }) {
  ...
  return h('div', { className: 'ya-filters', 'aria-label': 'audit filters' },
    h('label', { className: 'ya-search' }, h(IconSearchOutline16, { size: 16 }), h('span', { className: 'ya-sr' }, t('search')), h('input', { type: 'search', value: state.filters.query, disabled: busy, placeholder: t('search'), onChange: set('query') })),
    h('select', { value: state.filters.pluginId, disabled: busy, onChange: set('pluginId'), 'aria-label': t('allPlugins') }, h('option', { value: '' }, t('allPlugins')), h('option', { value: 'dsh-plugin-desktop/yootun-recruiter' }, '招聘'), h('option', { value: 'dsh-plugin-desktop/yootun-sales' }, '销售'), h('option', { value: 'dsh-plugin-desktop/yootun-supply-watch' }, '供应链'), h('option', { value: 'dsh-plugin-desktop/yootun-content-command' }, '内容运营')),
    h('select', { value: state.filters.actionCode, disabled: busy, onChange: set('actionCode'), 'aria-label': t('allActions') }, h('option', { value: '' }, t('allActions')), h('option', { value: 'content.publish.executed' }, '内容发布'), h('option', { value: 'sales.follow_up.executed' }, '销售跟进'), h('option', { value: 'supply.review.executed' }, '风险复核'), h('option', { value: 'recruiter.boss_sync.executed' }, '招聘同步')),
    h('select', { value: state.filters.outcome, disabled: busy, onChange: set('outcome'), 'aria-label': t('allResults') }, h('option', { value: '' }, t('allResults')), h('option', { value: 'succeeded' }, t('succeeded')), h('option', { value: 'partial' }, t('partial')), h('option', { value: 'failed' }, t('failed')), h('option', { value: 'accepted' }, t('accepted')), h('option', { value: 'requires_user_login' }, t('requiresLogin'))),
    h('select', { value: state.filters.timeRange, disabled: busy, onChange: set('timeRange'), 'aria-label': t('allTime') }, h('option', { value: '' }, t('allTime')), h('option', { value: '24h' }, t('time24h')), h('option', { value: '7d' }, t('time7d')), h('option', { value: '30d' }, t('time30d')))
  )
}
...
function Overlay({ t }) {
  ...
  useEffect(() => { if (!state.visible || waitingForTeam) return undefined; const controller = new AbortController(); requestRef.current = true; dispatch({ type: 'loading' }); const query = buildQuery({ scope: state.scope, teamId: state.teamId, ...state.filters }); void requestJson(query, controller.signal).then(value => dispatch({ type: 'loaded', value })).catch(error => { if (error?.name !== 'AbortError') dispatch({ type: 'error', error: error?.message }) }).finally(() => { if (!controller.signal.aborted) requestRef.current = false }); return () => { controller.abort(); requestRef.current = false } }, [state.visible, state.scope, state.teamId, state.filters, waitingForTeam])
  ...
  const refresh = async () => { if (requestRef.current) return; requestRef.current = true; dispatch({ type: 'loading' }); try { const value = await requestJson(buildQuery({ scope: state.scope, teamId: state.teamId, ...state.filters })); dispatch({ type: 'loaded', value }) } catch (error) { dispatch({ type: 'error', error: error?.message }) } finally { requestRef.current = false } }
  const reload = async () => { if (requestRef.current) return; requestRef.current = true; dispatch({ type: 'loading' }); try { await retrySync(); const value = await requestJson(buildQuery({ scope: state.scope, teamId: state.teamId, ...state.filters })); dispatch({ type: 'loaded', value }) } catch (error) { dispatch({ type: 'error', error: error?.message }) } finally { requestRef.current = false } }
  const append = async () => { if (!workspace.page.nextCursor || requestRef.current) return; requestRef.current = true; dispatch({ type: 'loading', append: true }); try { const value = await requestJson(buildQuery({ scope: state.scope, teamId: state.teamId, ...state.filters }, workspace.page.nextCursor)); dispatch({ type: 'loaded', value, append: true }) } catch (error) { dispatch({ type: 'error', error: error?.message }) } finally { requestRef.current = false } }
  const interactionBusy = state.loading || state.appending
  const key = event => { if (event.key === 'Escape') { if (state.selectedId) dispatch({ type: 'select', id: null }); else closeOverlay() } }
  const header = h('header', { className: 'ya-header' }, h('div', null, h('h1', { id: 'ya-title' }, t('title')), h('p', null, t('subtitle'))), h('div', { className: 'ya-header-status' }, h('span', null, workspace.freshness.source === 'cache' ? t('cached') : workspace.freshness.syncedAt ? `${t('stale')} ${formatTime(workspace.freshness.syncedAt)}` : ''), h(IconButton, { label: t('refresh'), disabled: interactionBusy, onClick: refresh, icon: IconRefreshOutline16 }), h(IconButton, { label: t('close'), onClick: closeOverlay, icon: IconCloseOutline16 })))
  const scope = h('div', { className: 'ya-scope' }, h('div', { className: 'ya-segments', role: 'tablist' }, h('button', { type: 'button', role: 'tab', 'aria-selected': state.scope === 'self', disabled: interactionBusy, onClick: () => dispatch({ type: 'scope', value: 'self' }) }, t('self')), workspace.scopes.available.includes('team') ? h('button', { type: 'button', role: 'tab', 'aria-selected': state.scope === 'team', disabled: interactionBusy, onClick: () => dispatch({ type: 'scope', value: 'team' }) }, t('team')) : null), state.scope === 'team' && workspace.scopes.isSuperAdmin ? h('div', { className: 'ya-team' }, h('input', { value: state.teamQuery, disabled: interactionBusy, placeholder: t('searchTeam'), 'aria-label': t('searchTeam'), onChange: event => dispatch({ type: 'teamQuery', value: event.target.value }) }), h('select', { value: state.teamId, disabled: interactionBusy, 'aria-label': t('searchTeam'), onChange: event => dispatch({ type: 'team', value: event.target.value }) }, h('option', { value: '' }, t('chooseTeam')), state.teams.map(team => h('option', { key: team.id, value: team.id }, team.name)))) : workspace.scopes.currentTeam ? h('span', { className: 'ya-team-name' }, workspace.scopes.currentTeam.name) : null)
  let body
  if (state.loading && !state.workspace) body = h(Skeleton, { t })
  else {
    const empty = h('div', { className: 'ya-empty', role: 'status' }, h('span', null, filtered ? t('filteredEmpty') : t('empty')), filtered ? h('button', { type: 'button', disabled: interactionBusy, onClick: () => dispatch({ type: 'clear' }) }, t('clear')) : null)
...
module.exports = { apply, inject: ['slots', 'locale'], __test: { normalizeWorkspace, buildQuery, mergePage, initialState, reducer, actionLabel, surfaceLabel, effectOutcomeLabel, outcomeLabel } }
