...
const layoutCss = `.ya-shell{display:flex;flex-direction:column;grid-template-rows:none}.ya-workspace,.ya-empty,.ya-skeletons{flex:1;min-height:0}.ya-workspace:not(.has-detail) .ya-detail{display:none}.ya-row>span:last-child{display:flex;align-items:center;gap:5px}.ya-pending{color:var(--dsw-alias-state-warn-primary);font-size:11px;white-space:nowrap}`
function apply(ctx) {
