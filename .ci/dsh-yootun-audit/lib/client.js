window.__ModuleLoader__.load({
  ...
  factory: (require) => { var module = { exports: {} }; var exports = module.exports;
    ...
    const actionLabel = code => ({ 'recruiter.requirement.created': '创建招聘岗位', 'recruiter.requirement.updated': '更新招聘岗位', 'recruiter.candidate_analysis.saved': '保存候选分析', 'recruiter.boss_sync.executed': '同步招聘数据', 'sales.lead.created': '创建销售线索', 'sales.lead.updated': '更新销售线索', 'sales.follow_up.created': '创建跟进动作', 'sales.follow_up.executed': '执行跟进动作', 'supply.risk.created': '创建供应风险', 'supply.risk.updated': '更新供应风险', 'supply.review.created': '创建风险复核', 'supply.review.executed': '执行风险复核', 'content.review.updated': '更新文章审核', 'content.platforms.updated': '更新发布渠道', 'content.publish.executed': '执行内容发布', 'knowledge.memory.remembered': '保存知识记忆', 'media.upload.completed': '完成媒体上传' }[code] || code || '—')
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = OVERLAY_ID; style.textContent = css; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-audit: styles')
    ...
    module.exports = { apply, inject: ['slots', 'locale'], __test: { normalizeWorkspace, buildQuery, mergePage, reducer } }
