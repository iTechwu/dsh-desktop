window.__ModuleLoader__.load({
  ...
  factory: (require) => { var module = { exports: {} }; var exports = module.exports;
    ...
    async function requestJson(query, signal) { const response = await fetch(`${PATH}?${query}`, { credentials: 'same-origin', redirect: 'error', signal: AbortSignal.any([signal, AbortSignal.timeout(REQUEST_TIMEOUT_MS)].filter(Boolean)), headers: { Accept: 'application/json' } }); if (!response.ok) throw new Error(`audit_${response.status}`); return response.json() }
    ...
    function Filters({ state, dispatch, t, busy }) {
      ...
      return h('div', { className: 'ya-filters', 'aria-label': 'audit filters' },
        ...
        h('select', { value: state.filters.outcome, disabled: busy, onChange: set('outcome'), 'aria-label': t('allResults') }, h('option', { value: '' }, t('allResults')), h('option', { value: 'succeeded' }, t('succeeded')), h('option', { value: 'partial' }, t('partial')), h('option', { value: 'failed' }, t('failed')), h('option', { value: 'accepted' }, t('accepted'))),
    ...
    const statusColorCss = `.ya-overlay{--ya-error-text:color-mix(in srgb,var(--dsw-alias-state-error-primary) 60%,var(--dsw-alias-label-primary));--ya-warning-text:color-mix(in srgb,var(--dsw-alias-state-warn-primary) 60%,var(--dsw-alias-label-primary))}.ya-outcome.is-failed,.ya-metric.is-failed strong{color:var(--ya-error-text)}.ya-pending,.ya-metric.is-pending strong{color:var(--ya-warning-text)}`
    function apply(ctx) {
    ...
    module.exports = { apply, inject: ['slots', 'locale'], __test: { normalizeWorkspace, buildQuery, mergePage, initialState, reducer, actionLabel, surfaceLabel, effectOutcomeLabel } }
