...
test('every Yootun business overlay participates in the exclusive accessible dialog contract', async () => {
  for (const plugin of overlayPlugins) {
    ...
    assert.match(source, /let lastTrigger = null/u, plugin)
    assert.match(source, /event\?\.currentTarget \|\| document\.activeElement/u, plugin)
    assert.match(source, /requestAnimationFrame\(\(\) => lastTrigger\?\.focus\?\.\(\)\)/u, plugin)
    assert.match(source, /tabIndex: -1/u, plugin)
    assert.match(source, /shellRef\.current\?\.focus\?\.\(\)/u, plugin)
  }
})
