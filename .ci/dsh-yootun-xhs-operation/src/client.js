...
const semanticCss = '.yxh-progress-fill{background:var(--dsw-alias-brand-primary)}'
...
function apply(ctx) {
  ...
  ctx.effect(() => { const style = document.createElement('style'); style.dataset.plugin = '@dofe/dsh-yootun-xhs-operation'; style.textContent = css + responsiveCss + semanticCss; document.head.appendChild(style); return () => style.remove() }, 'dofe-yootun-xhs-operation: styles')
