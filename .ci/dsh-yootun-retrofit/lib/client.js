  ...
  factory: (require) => {
    ...
    function InlineError({ t, busy, onRetry }) {
      return h('div', { className: 'yro-inline-error', role: 'alert' },
        ...
        h('div', null, h('strong', null, t('errorTitle')), h('p', null, t('errorBody'))),
        h('button', { type: 'button', disabled: busy, onClick: onRetry }, h(IconRefreshOutline16, { size: 14 }), t('search')))
    }
    function Overlay({ t }) {
      ...
      const load = async action => {
        ...
        try {
          ...
          setRequestError(true)
          setData(previous => previous || { status: 'error' })
          return null
        } finally {
      ...
      if (requestError && data?.status === 'ready') {
        body = h(React.Fragment, null, h(InlineError, { t, busy, onRetry: () => void load('refresh') }), body)
      }
