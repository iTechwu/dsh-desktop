window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function sessionFreshness(sessionCheckedAt, t, { now = Date.now() } = {}) {
    ...
    // 观众画像四维度（gender/age/province/city_level）：主要分类 + 加权占比 + 参与作品数
    //（UI 优化方案 §5.4：标签使用「主要 ×」口径，页面只显示结果及参与作品数）。
    // gender/age 的接口枚举（male/41-50 等）经 ui-format 中文化，内部枚举不进页面（验收 P1）。
    function audienceBlocks(analysis, t) {
      const audience = analysis?.audience
      if (!audience) return [h2('p', { className: 'ydo-hint' }, t('none'))]
      const labels = { gender: t('mainGender'), age: t('mainAge'), province: t('mainRegion'), city_level: t('cityLevel') }
      const keyFormatters = {
        gender: key => genderLabel(key, t),
        age: key => formatAgeBucket(key, t),
      }
      const blocks = []
      for (const [name, label] of Object.entries(labels)) {
        const dimension = audience.dimensions?.[name]
        if (!dimension) continue
        const formatKey = keyFormatters[name] || (key => key)
        const top = (dimension.distributions || []).slice(0, 3)
        const main = top.length
          ? top.map(item => `${formatKey(item.key)} ${pct(item.pct)}`).join('、')
          : t('dataInsufficient')
        blocks.push(h2('p', { key: name, className: 'ydo-hint' },
          `${label}：${main}`,
          dimension.sampleWorkCount
            ? ` · ${t('weightedSample').replace('{count}', String(dimension.sampleWorkCount))}`
            : ` · ${t('dataInsufficient')}`))
      }
      return blocks
    }

    function trafficBlock(analysis, t) {
      const traffic = analysis?.traffic
      const top = (traffic?.distributions || []).slice(0, 3)
      if (!top.length) return null
      // 展示名单一来源 ui-format 的 trafficSourceLabel：历史脏 label（与 key 相同或裸枚举
      // 形态）不直接展示，未知 key 兜底「其他来源」（验收 P1——不复刻第二套映射）。
      return h2('p', { className: 'ydo-hint' },
        `${t('mainTrafficSource')}：${top
          .map(item => `${trafficSourceLabel({ source_key: item.key, source_label: item.sourceLabel }, t)} ${pct(item.pct)}`)
          .join('、')}`)
    }

    function hotwordsBlock(analysis, t) {
      const hotwords = analysis?.hotwords
      const list = hotwords?.hotwords || []
      if (!list.length) return null
      return h2('div', null,
        h2('h4', { className: 'ydo-hint' }, t('hotwords')),
        h2('div', { className: 'ydo-tags' },
          ...list.map(item => h2('span', {
            key: item.word,
            className: `ydo-tag${item.stale ? ' ydo-tag-stale' : ''}`,
            title: `${t('collectedAt')}: ${item.collectedAt || '—'}${item.stale ? ` · ${t('hotwordStale')}` : ''}`,
          }, item.word, item.stale ? ` ${t('hotwordStaleBadge')}` : ''))))
    }
    ...
    function contentMetricNote(item, t) {
      // 第三段「状态/覆盖率」：覆盖率缺失或为 0 → 数据不足；0<x<100 → 部分数据 + 覆盖率；
      // 覆盖完整 → 只显示覆盖率，让结构与数值说话。
      ...
      return `${t('coverage')} ${pct(item.coveragePct)}`
    }
      ...
      return h2('div', { className: 'ydo-ov-table', role: 'table', 'aria-label': t('accountHotWorks') },
        ...
        ...works.map((work, index) => {
          ...
          return h2('div', { key: work.workId, className: 'ydo-ov-tr ydo-ov-tr-hot-rank', role: 'row' },
            h2('span', { className: 'ydo-ov-rankcell', role: 'cell' }, work.rank ?? index + 1),
            h2('div', { className: 'ydo-ov-hot-title', role: 'cell' },
              h2('button', {
                type: 'button', className: 'ydo-ov-work-link',
                onClick: () => onOpenWork && onOpenWork(work),
                title: work.title || work.workId,
              }, work.title || work.workId)),
            h2('span', { role: 'cell' }, formatDateTime(work.publishTime)),
            h2('span', { className: 'ydo-ov-num', role: 'cell' }, count(work.playCount)),
            h2('span', { className: 'ydo-ov-num', role: 'cell' }, pct(work.engagementRatePct)),
            h2('div', { className: 'ydo-ov-basis', role: 'cell', title: work.basis || '' },
              ...(lines.length
    ...
    function AnalysisPage({
      ...
      return h2('div', { className: 'ydo-an-page' },
        ...
        account ? h2('header', { className: 'ydo-an-head' },
          ...
          h2('p', { className: 'ydo-hint' },
            `${t('sessionFreshnessCol')}：${account.sessionCheck?.state === 'verified' ? t('sessionCheckValid') : account.sessionCheck?.state === 'stale' ? t('sessionStale') : t('sessionUnknown')}`),
        ...
        account ? h2('div', { className: 'ydo-ov-panels' },
          ...
          h2('section', { className: 'ydo-ov-panel' },
            ...
            // 阶段 3 开放：加权画像/流量/热词全部直接渲染接口字段（零口径计算）。
            h2('div', { className: 'ydo-an-audience' },
              ...audienceBlocks(analysis, t),
              trafficBlock(analysis, t)),
            hotwordsBlock(analysis, t))) : null,
