...
async function mutateSupplyState(path: string, value: unknown, now: string): Promise<SupplyState> { const previous = mutationQueues.get(path) ?? Promise.resolve(); let release: (() => void) | undefined; const current = new Promise<void>(resolve => { release = resolve }); const chain = previous.then(() => current); mutationQueues.set(path, chain); await previous; try { const next = mutate(await readSupplyState(path, now), value, now); await writeState(path, next); return next } finally { release?.(); if (mutationQueues.get(path) === chain) mutationQueues.delete(path) } }
