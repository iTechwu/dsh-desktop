...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    title: 'Yootun-Agent Recovery Assistant',
    fallbackBody: 'The recovery information could not be read. Quit and start Yootun-Agent again.',
    ...
    confirmSafeModeBody: 'Yootun-Agent will create an isolated DSH home without reading the normal DSH data directory. Your normal Profiles and data are not changed. The temporary environment is deleted on the next normal launch after you leave Safe Mode.',
    ...
    configurationFilesBody: 'View or edit the current Profile and shared Harness-home configuration. Restart Yootun-Agent after making changes.',
    ...
    confirmRestoreDefaultDirectoryBody: 'Sensteed Agent will use the default data directory for this system after restart. The current data directory will not be deleted.',
    ...
    confirmCreateDefaultDirectoryBody: 'Sensteed Agent will create a new environment at the default path and use it after restart. The current data directory will not be deleted.',
    confirmCreateDefaultDirectoryAction: 'Create and restart',
    confirmFactoryReset: 'Factory reset Sensteed Agent?',
    confirmFactoryResetMessage: 'Remove current DSH data and initialize a new environment?',
    confirmFactoryResetBody: currentDirectory => `The following directory will be moved to the operating-system Trash or Recycle Bin:\n\n${currentDirectory}\n\nProfiles, plugins, settings, credentials, sessions, attachments, and workspace records stored there will be removed from Sensteed Agent. Project files outside this directory are not deleted. Sensteed Agent will restart and create a clean default Profile.`,
    ...
    desktopVersion: 'Yootun-Agent version',
    pluginCount: 'Plugins',
    ...
    confirmDisableBody: 'Nothing is deleted. The plugin, its version declaration and its configuration all stay in the current Profile — Yootun-Agent simply will not load it on the next start. You can enable it again here at any time.',
    confirmDisableAction: 'Disable plugin',
    confirmEnable: 'Enable this plugin?',
    confirmEnableBody: 'Yootun-Agent will load this plugin again on the next start. If this plugin caused the startup failure, the recovery assistant will open again and you can disable it once more.',
    confirmEnableAction: 'Enable plugin',
    confirmRollback: 'Roll back this configuration?',
    confirmRollbackBody: capturedAt => `This immediately restores the current Profile plus the checkpointed settings.yaml and Harness-home patch captured at ${capturedAt}, including which plugins are enabled. After restarting, Yootun-Agent will use the rolled-back configuration.`,
    confirmRollbackAction: 'Roll Back',
    uninstalledSuccess: 'The plugin was removed from the current Profile. Restart Yootun-Agent to use the updated plugin configuration.',
    disabledSuccess: 'The plugin is disabled and still installed. Restart Yootun-Agent to start without it; nothing was removed from the Profile.',
    enabledSuccess: 'The plugin is enabled again. Restart Yootun-Agent to load it.',
    rollbackSuccess: slotId => `Rolled back to ${slotId}. Restart Yootun-Agent to use this configuration; the first healthy start after rollback will preserve all three existing slots.`,
    profileSelectedSuccess: 'This Profile is now selected. Restart Yootun-Agent to use it.',
  ...
  zh: {
    title: 'Yootun-Agent 恢复助手',
    fallbackBody: '无法读取恢复信息。请退出并重新启动 Yootun-Agent。',
    reason: '进入恢复模式的原因',
    ...
    confirmSafeModeBody: 'Yootun-Agent 将创建一个不读取现有 DSH 数据目录的独立 DSH Home，不会修改原有 Profile 和数据。退出安全模式后，下一次普通启动会自动删除临时环境。',
    ...
    configurationFilesBody: '查看或编辑当前 Profile 与 DSH home 的共享配置。修改后需要重新启动 Yootun-Agent。',
    ...
    confirmRestoreDefaultDirectoryBody: 'Sensteed Agent 将改为使用当前系统的默认数据目录。当前数据目录不会被删除。',
    ...
    confirmCreateDefaultDirectoryBody: 'Sensteed Agent 将在默认路径创建一个全新的环境并重启。当前数据目录不会被删除。',
    confirmCreateDefaultDirectoryAction: '新建并重启',
    confirmFactoryReset: '恢复 Sensteed Agent 出厂设置？',
    confirmFactoryResetMessage: '移除当前 DSH 数据并重新初始化？',
    confirmFactoryResetBody: currentDirectory => `以下目录将被移入系统废纸篓或回收站：\n\n${currentDirectory}\n\n其中保存的 Profile、插件、设置、凭据、会话、附件和工作区记录都会从 Sensteed Agent 中移除；不会删除此目录以外的项目文件。随后 Sensteed Agent 会重启并创建干净的默认 Profile。`,
    ...
    desktopVersion: 'Yootun-Agent 版本',
    pluginCount: '插件',
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 保存的当前 Profile、settings.yaml 与 DSH 数据目录补丁（包括插件的启用状态）；重启后，Yootun-Agent 将使用检查点中的配置。`,
    confirmRollbackAction: '回滚',
    uninstalledSuccess: '插件已从当前 Profile 中卸载。请重启 Yootun-Agent 以使用更新后的插件配置。',
    disabledSuccess: '插件已禁用，安装内容仍然保留。重启 Yootun-Agent 后将不再加载；Profile 中没有任何内容被删除。',
    enabledSuccess: '插件已重新启用。重启 Yootun-Agent 后将重新加载。',
    rollbackSuccess: slotId => `已回滚到${slotId}。请重启 Yootun-Agent 以使用该配置；回滚后的第一次健康启动会保留现有三个槽位。`,
    profileSelectedSuccess: '已设为当前 Profile。请重启 Yootun-Agent 以使用该 Profile。',
