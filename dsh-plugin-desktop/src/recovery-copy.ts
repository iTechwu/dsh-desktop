...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    ...
    confirmSafeModeBody: 'Yootun-Agent will create an isolated DSH home without reading ~/.dsh. Your normal Profiles and data are not changed. The temporary environment is deleted on the next normal launch after you leave Safe Mode.',
  ...
  zh: {
    ...
    confirmSafeModeBody: 'Yootun-Agent 将创建一个不读取 ~/.dsh 的独立 DSH Home，不会修改原有 Profile 和数据。退出安全模式后，下一次普通启动会自动删除临时环境。',
