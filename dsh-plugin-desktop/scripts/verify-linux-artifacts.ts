...
export function verifyLinuxArtifacts(
  ...
  const artifactStem = `${DESKTOP_ARTIFACT_PREFIX}-${options.version}`
  const appImagePath = join(distDir, `${artifactStem}-x86_64.AppImage`)
  const debPath = join(distDir, `${artifactStem}-amd64.deb`)
  const applicationPath = join(distDir, 'linux-unpacked', DESKTOP_ARTIFACT_PREFIX.toLowerCase())
