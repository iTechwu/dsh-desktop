...
describe('Yootun content command route', () => {
  it('persists a brief and keeps publishing explicitly confirmed', async () => {
    const root = await mkdtemp(join(tmpdir(), 'yootun-content-')); const statePath = join(root, 'state.json')
    ...
    const queued = response(); await handleYootunContentCommandRequest(request('POST', JSON.stringify({ action: 'queue_publish', briefId, summary: '确认 GEO 内容后再交给发布适配器', idempotencyKey: 'content-publish-1' })), queued, 'http://127.0.0.1:43120', { statePath }); expect(queued.body().dashboard.pendingConfirmation).toBe(1)
    const replay = response(); await handleYootunContentCommandRequest(request('POST', JSON.stringify({ action: 'queue_publish', briefId, summary: '确认 GEO 内容后再交给发布适配器', idempotencyKey: 'content-publish-1' })), replay, 'http://127.0.0.1:43120', { statePath }); expect(replay.body().actions).toHaveLength(1)
