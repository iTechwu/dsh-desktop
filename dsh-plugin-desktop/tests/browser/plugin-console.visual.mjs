...
const formCss = source.match(/const formCss = `([\s\S]*?)`;/u)?.[1]
assert(css && cssOverrides && formCss, 'Plugin Console CSS bundle is missing')
...
for (const viewport of [
  ...
  for (const theme of themes) {
    ...
    await page.addStyleTag({ content: formCss })
