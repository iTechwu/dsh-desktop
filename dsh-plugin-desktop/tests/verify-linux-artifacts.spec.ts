...
import { DESKTOP_ARTIFACT_PREFIX } from '../src/product-identity.ts'

const ARTIFACT_STEM = `${DESKTOP_ARTIFACT_PREFIX}-2.0.0`
...
function fixture(version = '2.0.0'): {
  ...
  const appImagePath = join(dist, `${DESKTOP_ARTIFACT_PREFIX}-${version}-x86_64.AppImage`)
  const debPath = join(dist, `${DESKTOP_ARTIFACT_PREFIX}-${version}-amd64.deb`)
  const application = join(unpacked, DESKTOP_ARTIFACT_PREFIX.toLowerCase())
...
describe('Linux artifact verification', () => {
  ...
  it('rejects an AppImage from a different version', () => {
    ...
    expect(() => verifyLinuxArtifacts({ desktopRoot: value.root, version: '2.0.0' }))
      .toThrow(`${ARTIFACT_STEM}-x86_64.AppImage`)
  })
