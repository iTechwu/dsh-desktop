...
The installed application is named **Yootun-Agent Beta**.<!-- brand:plugin-identity:end --> The package provides the `dsh-plugin-desktop` executable and the `dsh-desktop` alias; the registered npm package name is the reliable `npx` entry.
...
This package is managed by the pnpm workspace at the repository root. The sibling `deepseek-harness/` checkout remains an independent upstream pnpm project and is included through the root pnpm workspace. Install and verify DSH Desktop from the repository root:
...
Beta version `2.0.10-beta.2` is written to `dsh-plugin-desktop\dist\Yootun-Agent-Beta-2.0.10-beta.2-x64-Setup.exe`; the unpacked application is `dsh-plugin-desktop\dist\win-unpacked\Yootun-Agent Beta.exe` for smoke testing.<!-- brand:plugin-artifact-setup:end -->
