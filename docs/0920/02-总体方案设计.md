...
>
> **SSO 定位**：`https://sso.ixicai.cn`（Web）/ `https://sso.ixicai.cn/api`（API 与 issuer）是全部自研项目的**唯一用户源和组织源**。用户与组织只建在 SSO；下游服务（models / knowledge / tools / montage / 桌面端）一律凭 SSO 签发的身份运行，禁止自建用户或组织体系、禁止绕过 SSO 直连飞书。
...
sequenceDiagram
    ...
    D->>B: shell.openExternal(sso.ixicai.cn/api/oauth/authorize?client_id=sensteed-desktop&redirect_uri=http://127.0.0.1:<port>/callback&code_challenge=..&state=..)
    B->>S: 授权页（已有飞书 connector 登录入口）
    U->>S: 飞书扫码确认
    S->>S: 首登：自动建户 + 绑定 feishuId + 校验 tenant_key 白名单
    S->>D: 302 http://127.0.0.1:<port>/callback?code&state
    D->>S: POST sso.ixicai.cn/api/oauth/token (code + code_verifier)
...
> 端点地址约定：SSO OIDC 各端点以 discovery 文档 `https://sso.ixicai.cn/api/.well-known/openid-configuration` 解析为准（`/oauth/authorize`、`/oauth/token` 等均在 `/api` 前缀下）；models 端点为 `https://ixicai.cn/api/*`。桌面端不硬编码端点，一律读 discovery。
