...
## 第 12 轮：业务工具调用契约与真实候选读取

时间：2026-09-04 06:30-06:53（Asia/Shanghai）

### 复现与根因

- 线索发现点击“已存候选”稳定显示“读取失败，请稍后重试”，冷启动后仍复现；日志记录 `lead_discovery_request_failed`。
- 使用既有 Models 凭据直接调用公共 MCP，只记录 HTTP 状态和结构字段：`lead_discovery_candidates_list` 返回 200、`isError=false`、0 条候选，证明服务、鉴权和数据仓可用。
- 当前 Harness 的 `ToolExecutionInput` 要求 `signal: AbortSignal`，执行入口立即读取 `signal.aborted`；扫描发现线索、销售、供应链、统一审批、内容、招聘、改装共 7 个 Host 的直接工具调用均遗漏该字段。
- 为七包先加入有效信号断言，全部按预期 RED；断言抛错还暴露 5 个 Host 引用了未定义的 `safeError`，审批双源均失败时会错误返回 `ready`。

### 修复与自动门禁

- Agent 工具包装沿用 Harness 传入的取消信号；Web 路由创建 60 秒超时信号，审批并发读取共享同一信号。
- 补齐 Host 异常回退，并将审批双源均失败的聚合状态改为 `error`；增加销售、供应链、审批、内容和招聘工具失败回归。
- 七包最终结果：线索 4/4、销售 6/6、供应链 6/6、审批 6/6、内容 5/5、招聘 8/8、改装 2/2，总计 37/37 PASS。
- `docker-helm.dofe.ai@8bd495c` 已推送 `origin/master`。

### 真实应用回归与部署判断

- `package:dir` 完成桌面构建、Electron 43.4.0 依赖组装与 fuses，生成时间 06:48；新包冷启动成功。
- “已存候选”从失败态恢复为计数 0 和“还没有已存候选”；供应链显示 0 风险/“暂无风险信号”，内容指挥显示 0 任务/“暂无内容任务”，统一审批显示 0 待确认/“暂无待确认动作”。
- 06:49 新进程至本轮结束没有新增 `yootun ... failed` 日志；未执行持久化检索、批准或发布，没有写入业务数据。
- 故障属于桌面内嵌插件调用契约，服务端探测健康，无需 Jenkins 部署。该目录包仅供即时回归，07:30 后仍按计划生成并验证双平台最终产物。

## 第 13 轮：改装检索递归失效保护

时间：2026-09-04 06:54-07:05（Asia/Shanghai）

### 真实故障与诊断

- 在重包中输入只读查询 `2024 SUV LED lighting upgrade` 后，桌面控制返回 `noWindowsAvailable`；主进程仍监听本地端口，但持续约 100% CPU、物理内存约 1.2 GiB，窗口长时间无法访问。
- `SIGTERM` 无法结束失去响应的明确进程，收集 2 秒系统采样后使用 `SIGKILL` 结束；没有删除文件或业务数据。
- 根因是改装 Host 注册 `yootun_retrofit_search` 后，以 `/retrofit/` 模糊匹配全部 schema，真实运行优先选中自身并无限递归。单测只提供外部假工具，未模拟注册后的完整工具列表。
- 公共 Custom Car MCP 的 `tools/list` 返回 12 个工具；计划、运行、内容、趋势、资产、周期与结果分页契约均不接受 `query`，因此不能把现有工具猜测为通用方案搜索。

### 修复与回归

- 先加入“自身工具 + 不兼容外部工具”用例，观察到错误返回 `ready`；随后只选择名称属于 Custom Car 且 schema 明确声明字符串 `query` 参数的外部工具，无兼容工具时返回 `unavailable`。
- 同时把返回文本限制为每项 20,000 字符、最多 30 项，防止未来兼容工具把超大文本直接渲染；定向测试 3/3 PASS。
- `docker-helm.dofe.ai@7d28070` 已推送 `origin/master`；再次完成 macOS 目录包构建和冷启动。
- 同一真实输入约 2 秒内显示“改装工具暂不可用”，窗口保持可用，主进程约 1% CPU，未新增改装失败日志。

### 部署判断

- 递归修复属于内嵌插件，不需要 Jenkins。公共 MCP 当前缺少兼容只读查询契约，本轮不修改服务端或用现有写工具替代；待服务端明确产品契约后再通过 Jenkins 部署与验收。
