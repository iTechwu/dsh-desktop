...
| AUD-002 | P1 | 候选修复已隔离验证，真实升级待执行 | `@xmanrui/dsh-im@2.3.0` 的 9 个消息渠道每 30 秒通过未认证 HTTP 调用 `host.describe` 并收到 403 | 独立 `DSH_HOME` 从 2.3.0 升级 4.9.1 后桌面完整启动并连续观察 67 秒，无新增 403；合成停用飞书机器人因无凭据只记录一次激活失败，不进入重试。降回 2.3.0 成功且配置哈希不变。真实 Profile 与机器人配置未修改，仍需带真实凭据的受控迁移验收 |
