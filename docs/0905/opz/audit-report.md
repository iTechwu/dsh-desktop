...
| AUD-003 | P1 | 已修复 | 多个业务覆盖层可叠加，关闭上层后露出前一层；隐藏层与主界面控件仍在辅助功能树中 | `docker-helm.dofe.ai@8c9f508` 实现 11 个面板互斥；`sensteed-agent@705ab05047` 和 `deepseek-harness@de9675462b` 分别覆盖桌面自有帧与兼容帧的背景隔离。真实包打开 Dashboard 后辅助功能树只保留当前弹窗 |
