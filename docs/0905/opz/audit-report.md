...
| AUD-003 | P1 | 部分修复 | 多个业务覆盖层可叠加，关闭上层后露出前一层；隐藏层与主界面控件仍在辅助功能树中 | `docker-helm.dofe.ai@8c9f508` 已实现 11 个面板互斥；真实包中 Dashboard -> Recruiter 后前者计数从 5 降为 0。背景控件仍在辅助功能树，初始焦点仍停在触发按钮 |
...
| AUD-010 | P2 | 待修复 | 打开业务对话框后焦点未进入面板 | 真实 Electron 可访问树显示焦点仍在后台启动按钮；Escape 可关闭并保留原焦点，但键盘用户需要先穿过背景控件 |
