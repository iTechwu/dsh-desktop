...
- 安装与启停/卸载/服务重启之间的协调，以及多个窗口之间的并发写入仍需补齐验证。当前页面的请求锁不代表 Host 已提供跨窗口互斥，已有文案、状态变量或静态门禁不能单独证明这些交互完成。安装进度窄屏截图发现浮动“回到顶部”按钮覆盖部分重启按钮，需修正位置并补充针对性遮挡验证；现有无障碍门禁不能证明所有浮动控件都无遮挡。
