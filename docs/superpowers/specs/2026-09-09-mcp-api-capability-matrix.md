...
- 无 Desktop 服务的独立 CLI 模式仍沿用旧脚本；非 Windows 重启、Windows 手动拉起和框架升级调度的跨平台支持与异步失败反馈仍需修复。框架升级状态读取的断连恢复与并发轮询，以及桌面托管框架应通过应用更新而非原地 npm 升级的所有者边界也仍需验证。上述浏览器测试拦截本地 API，未启动 PowerShell 或执行框架安装。
