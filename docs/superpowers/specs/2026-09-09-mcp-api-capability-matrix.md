...
- 上述浏览器测试拦截本地 API，未结束真实 Host、启动 PowerShell 或执行框架安装。Host 生命周期实现仍待修复：Plugin Console 的非 Windows `/restart` 只结束当前进程，`/framework-relaunch` 和框架升级调度依赖 Windows 脚本，且脚本写入/调度回调失败可能未反映在已返回的接受响应中。桌面已有 `DesktopActionsService` 和设置重启控制器，但原生确认、取消、响应先于退出以及跨窗口互斥必须按桌面所有者契约接入，不能把本轮客户端锁或 `ok: true` 当作真实重启成功证据。升级状态读取的断连恢复与并发轮询也仍需验证。
- 安装、技能详情页的重复入口及多个窗口之间的并发写入仍需补齐失败恢复与重复提交验证。当前页面的请求锁不代表 Host 已提供跨窗口互斥，已有文案、状态变量或静态门禁不能单独证明这些交互完成。
