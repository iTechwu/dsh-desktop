...
感谢你愿意参与 Sensteed Agent。这是一个社区项目，无论你是普通用户、插件作者还是开发者，都有适合你的贡献方式。
...
- 提交前运行 `pnpm check` 并保证全绿。
- 变更生产依赖后，运行 `pnpm --filter dsh-plugin-desktop verify:notices` 刷新第三方许可清单，并提交更新后的 `dsh-plugin-desktop/THIRD_PARTY_NOTICES.md`。
